...
// seedanceTaskAdaptor returns the local Seedance task adaptor when the platform
// names the Seedance Link channel type. Seedance is a code-registered Link
// protocol and is never served by the js task-plugin registry.
func seedanceTaskAdaptor(platform constant.TaskPlatform) channel.TaskAdaptor {
	if channelType, err := strconv.ParseInt(string(platform), 10, 64); err == nil && channelType == int64(constant.ChannelTypeSeedanceLink) {
