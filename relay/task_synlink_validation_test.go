...
func TestSynlinkOpaqueAssetRejectedBeforePricingAndHold(t *testing.T) {
	saveBillingConfig(t)
	...
	request.Content = append(request.Content, dto.ModelArkVideoContent{Type: "image_url", Role: common.GetPointer("reference_image"), ImageURL: &dto.VideoMediaURL{URL: "asset://foreign-asset"}})
	raw, err := common.Marshal(request)
	require.NoError(t, err)
	c, info := newTaskSubmitContext(t, "customer", `{"customer":"doubao-seedance-2-0-260128"}`)
	...
	common.SetContextKey(c, constant.ContextKeyChannelOtherSetting, dto.ChannelOtherSettings{VideoUpstreamProtocol: dto.VideoUpstreamProtocolSynlinkVideoV1})
...
func TestSynlinkUnverifiedCreateErrorsRemainUnknown(t *testing.T) {
	...
	for _, status := range []int{400, 401, 403, 404, 429, 500} {
		assert.Equal(t, relaycommon.TaskCreateOutcomeUnknown, taskCreateHTTPDisposition(info, status,
			[]byte(`{"error":{"code":"AuthenticationError","message":"Denied"}}`)))
	}
}
