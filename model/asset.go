...
func ListAssetsByUser(userID, offset, limit int, filters ...AssetListFilter) ([]Asset, int64, error) {
	...
	query := DB.Model(&Asset{}).Where("user_id = ? AND deleted_at = ?", userID, 0)
	if len(filters) > 0 {
		filter := filters[0]
		if filter.Status != "" {
...
func ChannelHasActiveAssetResources(channelID int) (bool, error) {
	...
	err := DB.Model(&RealPersonAuthorization{}).Where("channel_id = ? AND status NOT IN ?", channelID, []string{RealPersonAuthorizationConsentRejected, RealPersonAuthorizationExpired, RealPersonAuthorizationRevoked, RealPersonAuthorizationDeleted}).Count(&authorizationCount).Error
