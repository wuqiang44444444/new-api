...
func ListModelArkVideoTasks(userID, appID int, filter ModelArkTaskListFilter) ([]Task, int64, error) {
	if userID <= 0 || appID <= 0 {
		return []Task{}, 0, nil
	}
	if filter.PageNum <= 0 {
	...
	query := DB.Where(
		"user_id = ? AND app_id = ? AND client_protocol = ? AND client_deleted_at = ? AND created_at >= ? AND created_at < ?",
		userID,
		appID,
...
func (s TaskStatus) ToModelArkVideoStatus() string {
	switch s {
	case TaskStatusInProgress, TaskStatusReconciliationRequired:
		return "running"
	case TaskStatusSuccess:
		return "succeeded"
...
func ModelArkTaskStatuses(status string) ([]TaskStatus, bool) {
	...
	case "running":
		return []TaskStatus{TaskStatusInProgress, TaskStatusReconciliationRequired}, true
	case "succeeded":
		return []TaskStatus{TaskStatusSuccess}, true
	case "failed":
		return []TaskStatus{TaskStatusFailure, TaskStatusProviderContractFailure}, true
	case "cancelled":
