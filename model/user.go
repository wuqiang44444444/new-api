...
type User struct {
	...
	Email                string                     `json:"email" gorm:"index" validate:"omitempty,email,max=50"`
	...
	UsedQuota            int                        `json:"used_quota" gorm:"type:bigint;default:0;column:used_quota"` // used quota; cumulative statistic, must hold values beyond int32
...
func (user *User) HardDelete() error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := DeleteCurrentCustomerContractRulesWithTx(tx, user.Id); err != nil {
			return err
		}
