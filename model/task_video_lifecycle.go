...
func ProjectOpenAIVideo(task *Task) *dto.OpenAIVideo {
	...
	case TaskStatusProviderContractFailure:
		video.Error = &dto.OpenAIVideoError{Code: "provider_contract_failure", Message: "The provider result could not be delivered safely"}
...
func GetVisibleVideoTask(userID, appID int, taskID string, protocols ...string) (*Task, bool, error) {
...
func ListOpenAIVideoTasks(userID, appID int, after string, limit int, order string) ([]Task, bool, error) {
	if userID <= 0 || appID <= 0 {
	...
	query := DB.Where("user_id = ? AND app_id = ? AND client_protocol = ? AND client_deleted_at = ?", userID, appID, TaskClientProtocolOpenAIVideos, 0)
	if after != "" {
		var cursor Task
		err := DB.Select("id", "created_at").
			Where("user_id = ? AND app_id = ? AND task_id = ? AND client_protocol = ? AND client_deleted_at = ?", userID, appID, after, TaskClientProtocolOpenAIVideos, 0).
			First(&cursor).Error
		if err != nil {
			return nil, false, err
		}
