...
func LoadRemoteAssetCreateReplay(idempotency *AssetCreateIdempotency) (*Asset, *AssetBinding, bool, error) {
	...
	if err := DB.First(&existingBinding, "asset_id = ? AND user_id = ?", existing.AssetID, existing.UserID).Error; err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, nil, false, err
	}
	if existingBinding.ID == 0 {
		return &existingAsset, nil, true, nil
	}
	return &existingAsset, &existingBinding, true, nil
}

func CreateRemoteAssetWithQuota(asset *Asset, sourceURL string, sourceExpiresAt int64, binding *AssetBinding, idempotency *AssetCreateIdempotency, maxCount, createUnknownTTLSeconds int64) (*Asset, *AssetBinding, bool, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if asset.AuthorizationID != nil {
			...
			if authorization.UserID != asset.UserID || authorization.AppID != asset.AppID || authorization.EndUserSubjectHash == "" || authorization.EndUserSubjectHash != asset.EndUserSubjectHash || authorization.Status != RealPersonAuthorizationAuthorized || authorization.RevokedAt != 0 {
				return ErrAssetAuthorizationNotAuthorized
			}
		}
		if idempotency != nil {
		...
		if _, err := CreateAssetSourceTx(tx, asset, sourceURL, sourceExpiresAt); err != nil {
			return err
		}
		if binding != nil {
		...
		if idempotency != nil {
			...
			if binding == nil {
				idempotency.Status = AssetCreateIdempotencyComplete
			}
