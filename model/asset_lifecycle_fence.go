...
func channelHasActiveAssetResourcesTx(tx *gorm.DB, channelID int) (bool, error) {
	...
	err := tx.Model(&RealPersonAuthorization{}).
		Where("channel_id = ? AND status NOT IN ?", channelID, []string{RealPersonAuthorizationConsentRejected, RealPersonAuthorizationExpired, RealPersonAuthorizationRevoked, RealPersonAuthorizationDeleted}).
		Count(&count).Error
	return count > 0, err
}
