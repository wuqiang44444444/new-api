...
func TestUpstreamReferenceAmountCoreExample(t *testing.T) {
	...
	require.Len(t, group.ChannelDiscounts, 3)
	byChannel := make(map[int]ProviderChannelDiscountStatus)
	for _, status := range group.ChannelDiscounts {
		byChannel[status.ChannelId] = status
	}
	...
	modelRow := group.Models[0]
	require.NotNil(t, modelRow.OriginalAmount)
	assert.EqualValues(t, 3500, *modelRow.OriginalAmount)
	require.NotNil(t, modelRow.ReferenceAmount)
	assert.EqualValues(t, 3100, *modelRow.ReferenceAmount)
	require.Len(t, modelRow.Channels, 3)
	leafAmounts := make(map[int][2]int64)
	for _, leaf := range modelRow.Channels {
		require.NotNil(t, leaf.OriginalAmount)
		if leaf.ReferenceAmount != nil {
			leafAmounts[leaf.ChannelId] = [2]int64{*leaf.OriginalAmount, *leaf.ReferenceAmount}
		} else {
			leafAmounts[leaf.ChannelId] = [2]int64{*leaf.OriginalAmount, -1}
		}
	}
	assert.Equal(t, [2]int64{1000, 800}, leafAmounts[71])
	assert.Equal(t, [2]int64{2000, 1800}, leafAmounts[72])
	assert.Equal(t, [2]int64{500, 500}, leafAmounts[73])
...
// settlement: it neither claims an amount nor blocks its parent's totals.
func TestUpstreamReferenceAmountIgnoresTestOnlyChannelForTotals(t *testing.T) {
	db := setupBillingReconciliationTestDB(t)
	july := channelDiscountPeriod(time.July, 2026)
	require.NoError(t, db.Create(&[]Channel{
	...
	group := summary.Groups[0]
	require.NotNil(t, group.OriginalAmount)
	assert.EqualValues(t, 1000, *group.OriginalAmount)
	require.NotNil(t, group.ReferenceAmount)
	assert.EqualValues(t, 800, *group.ReferenceAmount)
	assert.True(t, group.ReferenceKnown)
	assert.Equal(t, 0, group.DiscountPendingChannels)
	require.Len(t, group.Models, 1)
	modelRow := group.Models[0]
	require.Len(t, modelRow.Channels, 2)
	testsOnly := modelRow.Channels[1]
	assert.Equal(t, 77, testsOnly.ChannelId)
	assert.True(t, testsOnly.UsageOnly)
	assert.Nil(t, testsOnly.OriginalAmount)
	require.NotNil(t, modelRow.OriginalAmount)
	assert.EqualValues(t, 1000, *modelRow.OriginalAmount)
	require.NotNil(t, modelRow.ReferenceAmount)
	assert.EqualValues(t, 800, *modelRow.ReferenceAmount)
}
...
func TestUpstreamAmountsSumRoundedEvidenceRows(t *testing.T) {
 ...
 for _, m := range group.Models {
  ...
  for _, leaf := range m.Channels { require.NotNil(t,leaf.OriginalAmount); require.NotNil(t,leaf.ReferenceAmount); leafOriginal += *leaf.OriginalAmount; leafReference += *leaf.ReferenceAmount }
  assert.Equal(t,leafOriginal,*m.OriginalAmount); assert.Equal(t,leafReference,*m.ReferenceAmount)
  original += leafOriginal; reference += leafReference
 }
 assert.Equal(t,original,*group.OriginalAmount); assert.Equal(t,reference,*group.ReferenceAmount)
}
