...
func TestGetUserBillingStatementWithClickHouse(t *testing.T) {
	...
	logs := []Log{
		...
		{Id: 3, UserId: 7, CreatedAt: 1250, Type: LogTypeConsume, TokenId: 11, TokenName: "task-key", ModelName: "model-a", CompletionTokens: 50, Quota: 200, Other: `{"task_id":"supplement-task"}`},
	}
	...
	assert.EqualValues(t, 300, items[0].CompletionTokens)
	assert.EqualValues(t, 1200, items[0].GrossQuota)
	assert.EqualValues(t, 300, items[0].RefundQuota)
	assert.EqualValues(t, 900, items[0].NetQuota)
	assert.InDelta(t, 2, items[0].AverageUseTimeSeconds, 0.0001)
	assert.EqualValues(t, 1, summary.Requests)
	assert.EqualValues(t, 300, summary.TotalTokens)
	assert.EqualValues(t, 900, summary.NetQuota)

	breakdownItems, breakdownSummary, err := GetUserBillingStatementBreakdown(7, 1000, 1300, 0, "")
	require.NoError(t, err)
	require.Len(t, breakdownItems, 1)
	assert.EqualValues(t, 1, breakdownItems[0].Requests)
	assert.EqualValues(t, 1200, breakdownItems[0].GrossQuota)
	assert.EqualValues(t, 200, breakdownItems[0].UnallocatedAdjustmentQuota)
	assert.EqualValues(t, summary.GrossQuota, breakdownSummary.GrossQuota)
}
