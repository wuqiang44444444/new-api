...
func TestPendingRealPersonAuthorizationsCanShareNullReceiptHash(t *testing.T) {
	truncateTables(t)
	authorizations := []RealPersonAuthorization{
		{UserID: 101, ConsentTokenHash: "consent-one", Status: RealPersonAuthorizationAwaitingConsent},
		{UserID: 101, ConsentTokenHash: "consent-two", Status: RealPersonAuthorizationAwaitingConsent},
	}
	require.NoError(t, DB.Create(&authorizations).Error)
	assert.Nil(t, authorizations[0].ReceiptTokenHash)
	assert.Nil(t, authorizations[1].ReceiptTokenHash)
}
...
func TestRemoteAssetCreateIdempotencyReplaysWithoutDuplicateAsset(t *testing.T) {
	...
	_, fingerprint := seedAssetLifecycleChannel(t, 9, "credential", "relay_assets")
	newRecords := func(requestHMAC string) (*Asset, *AssetBinding, *AssetCreateIdempotency) {
		...
		binding := &AssetBinding{UserID: 404, ChannelID: 9, CredentialFingerprint: fingerprint, UpstreamProfile: "relay_assets", RequestedModel: "video-model", Status: AssetBindingStatusCreating}
	...
	created, _, replay, err := CreateRemoteAssetWithQuota(asset, binding, idempotency, 10, 300)
	require.NoError(t, err)
	...
	asset, binding, idempotency = newRecords("request-a")
	replayed, _, replay, err := CreateRemoteAssetWithQuota(asset, binding, idempotency, 10, 300)
	require.NoError(t, err)
	assert.True(t, replay)
	assert.Equal(t, created.ID, replayed.ID)

	asset, binding, idempotency = newRecords("request-b")
	_, _, _, err = CreateRemoteAssetWithQuota(asset, binding, idempotency, 10, 300)
...
func TestRemoteAssetCreateRechecksRealPersonAuthorizationInsideTransaction(t *testing.T) {
	...
	_, fingerprint := seedAssetLifecycleChannel(t, 9, "credential", "ark_assets")
	authorization := RealPersonAuthorization{
		UserID: 405, Status: RealPersonAuthorizationRevoked,
		ConsentTokenHash: "revoked-authorization-consent", RevokedAt: common.GetTimestamp(),
	}
	...
	binding := &AssetBinding{UserID: 405, ChannelID: 9, CredentialFingerprint: fingerprint, UpstreamProfile: "ark_assets", RequestedModel: "video-model", Status: AssetBindingStatusCreating}
