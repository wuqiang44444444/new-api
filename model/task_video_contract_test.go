...
func TestModelArkListEnforcesProtocolSevenDayWindowAndOfficialFilters(t *testing.T) {
	...
	excluded := []Task{
		{TaskID: "too-old", UserId: userID, ClientProtocol: TaskClientProtocolModelArkV3, Status: TaskStatusSuccess, CreatedAt: now - ModelArkTaskListWindowSeconds - 1},
		{TaskID: "future", UserId: userID, ClientProtocol: TaskClientProtocolModelArkV3, Status: TaskStatusSuccess, CreatedAt: now},
		{TaskID: "openai", UserId: userID, ClientProtocol: TaskClientProtocolOpenAIVideos, Status: TaskStatusSuccess, CreatedAt: now - 1},
		{TaskID: "deleted", UserId: userID, ClientProtocol: TaskClientProtocolModelArkV3, ClientDeletedAt: now - 1, Status: TaskStatusSuccess, CreatedAt: now - 1},
	}
	for i := range excluded {
		require.NoError(t, DB.Create(&excluded[i]).Error)
	}

	tasks, total, err := ListModelArkVideoTasks(userID, ModelArkTaskListFilter{
	...
	for _, status := range []string{"queued", "running", "succeeded", "failed", "cancelled", "expired"} {
		...
		filtered, filteredTotal, err := ListModelArkVideoTasks(userID, ModelArkTaskListFilter{
...
func TestTaskCancellationJournalSerializesRequestsAndTerminalTransition(t *testing.T) {
	...
	first, err := BeginTaskCancellation(task.UserId, task.TaskID, task.ClientProtocol)
	require.NoError(t, err)
	require.True(t, first.ShouldCall)
	assert.False(t, first.AlreadyPending)

	second, err := BeginTaskCancellation(task.UserId, task.TaskID, task.ClientProtocol)
	require.NoError(t, err)
