...
func GetOpenAIChatCapabilities(modelName, reasoningEffort string) OpenAIChatCapabilities {
	...
	// Opt in this exact rolling model ID without inferring a GPT generation.
	if !isGPT5Model && modelName != "gpt-chat-latest" && !isOpenAIModelSnapshot(modelName, "gpt-6-astra") {
		return capabilities
	}
	capabilities.UseMaxCompletionTokens = true
