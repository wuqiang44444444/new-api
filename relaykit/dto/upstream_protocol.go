...
const (
	...
	// Retired: recognized only for already-created tasks and their billing facts.
...
func ValidateVideoUpstreamProtocol(p VideoUpstreamProtocol) error {
	// Recognition is retained only for frozen task polling/settlement. New
	// configuration and submissions must use the provider's current protocol.
	if p == VideoUpstreamProtocolFunCloudSeedance || p == VideoUpstreamProtocolMoxingMediaTaskV1 {
...
func (p VideoUpstreamProtocol) TransportPaths(providerModel string) (string, string) {
	switch p {
	case VideoUpstreamProtocolMoxingMediaTaskV1:
		return "", "/v1/media/tasks/{task_id}"
	case VideoUpstreamProtocolSynlinkVideoV1:
	...
	case VideoUpstreamProtocolFunCloudSeedance:
		...
		return "", "/api/v2/open/aigc/{task_id}"
	default:
		return "", ""
	}
}
