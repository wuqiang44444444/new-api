...
func Current() Config {
	...
	return Config{
		...
		ConsentEvidenceHMACKey:      os.Getenv("ASSET_CONSENT_EVIDENCE_HMAC_KEY"),
...
func (c Config) ConsentReady() bool {
	if !c.Enabled || !c.RealPersonEnabled || len(c.ConsentEvidenceHMACKey) < 32 || !persistentCryptoSecretConfigured() {
		return false
	}
