...
func TestConsentReadyRequiresHTTPSAndStrongEvidenceKey(t *testing.T) {
	t.Setenv("SESSION_SECRET", "stable-test-session-secret")
	config := Config{
		Enabled: true, RealPersonEnabled: true,
		PublicBaseURL: "https://api.example", ConsentEvidenceHMACKey: "0123456789abcdef0123456789abcdef",
	}
	assert.True(t, config.ConsentReady())
	config.PublicBaseURL = "http://api.example"
	assert.False(t, config.ConsentReady())
	config.PublicBaseURL = "https://api.example"
	config.ConsentEvidenceHMACKey = "short"
	assert.False(t, config.ConsentReady())
	config.ConsentEvidenceHMACKey = "0123456789abcdef0123456789abcdef"
