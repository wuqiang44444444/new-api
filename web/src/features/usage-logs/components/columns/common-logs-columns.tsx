...
function buildTypeDetailSegments(
  ...
  if (isTieredExpr && other.is_task) {
    ...
    if (tieredSummary?.conditionResultsUnavailable) {
      segments.push({
        text: t('Historical condition results are unavailable.'),
        muted: true,
      })
      ...
      const otherEntries = tieredSummary.priceEntries
        ...
        .map(
          (entry) =>
            `${t(entry.shortLabel)} ${entry.unit === 'request' ? `${formatPriceCompact(entry.price)}/${t('request')}` : formatPrice(entry.price)}`
        )
