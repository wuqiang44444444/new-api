...
it('shows the estimated token cost in the selected currency while token quantities stay unchanged', async () => {
  ...
  expect(await screen.findByText('Estimated cost: $2')).toBeVisible()
  await selectCurrency('Site currency (CNY)')
...
it('submits the full time expression at the selected instant without changing saved pricing', async () => {
  const expression =
    'tier("base", p*1.5+cr*0.05+c*4.5)*(weekday("Asia/Shanghai")>=1 && weekday("Asia/Shanghai")<=5 && hour("Asia/Shanghai")>=9 ? 2 : 1)/6.71'
      ...
      data: [
        {
          key: 'estimator',
          evaluation: {
            raw_cost_usd: 0.4947839046199702,
            quota: 247392,
            matched_tier: 'base',
            pricing_time: '2026-09-07T09:00:00+08:00',
            usage_semantic: 'openai',
            normalized_usage: { p: 800000, cr: 200000, c: 100000 },
            request_rules: [
              {
                cond: 'weekday("Asia/Shanghai") >= 1',
                multiplier: 2,
                matched: true,
              },
            ],
  ...
  const saved = await commit(editor.ref)
  const time = screen.getByLabelText(/Simulation time/)
  fireEvent.change(time, { target: { value: '2026-09-07T09:00:00' } })
      ...
      '/api/option/billing-expression/preview',
      {
        items: [
          expect.objectContaining({
            expression: saved?.billingExpr,
            sample: expect.objectContaining({
              pricing_time: new Date('2026-09-07T09:00:00').toISOString(),
            }),
          }),
        ],
      }
  ...
  expect(await screen.findByText(/Estimated cost: \$0.49478/)).toBeVisible()
  expect(screen.getByText(/Normalized usage.*800,000/)).toBeVisible()
  expect(screen.getByText(/weekday.*Matched/)).toBeVisible()
  expect(await commit(editor.ref)).toEqual(saved)
  fireEvent.change(time, { target: { value: '2026-09-12T10:00:00' } })
  await waitFor(() =>
    expect(post).toHaveBeenLastCalledWith(
      '/api/option/billing-expression/preview',
      {
        items: [
          expect.objectContaining({
            sample: expect.objectContaining({
              pricing_time: new Date('2026-09-12T10:00:00').toISOString(),
            }),
          }),
        ],
      }
...
it.each([
  ['tier("base", p * 2 + 10000)', 0.01],
  ['tier("base", p * 2)', 0],
])(
  'evaluates zero usage for %s without changing the saved expression',
  async (expression, cost) => {
    const post = vi.spyOn(api, 'post').mockResolvedValue({
      data: {
        success: true,
        data: [
          {
            ...
            evaluation: {
              ...
              quota: cost * 500000,
              matched_tier: 'base',
              pricing_time: '2026-09-10T10:00:00+08:00',
              normalized_usage: { p: 0, c: 0 },
              usage_semantic: 'openai',
            },
          },
        ],
    ...
    expect(await screen.findByText(/Estimated cost:/)).toHaveTextContent(
      `$${cost}`
    )
    expect(await commit(editor.ref)).toEqual(saved)
  }
