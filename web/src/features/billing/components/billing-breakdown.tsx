...
*/
import { CircleDollarSign, DatabaseZap, Layers3, Zap } from 'lucide-react'
  ...
  return (
    <section className='flex flex-col gap-2'>
      ...
      <div className='grid gap-2 md:grid-cols-3'>
        {summary.cache && (
          <Card size='sm'>
            ...
            <CardContent className='flex flex-col gap-2'>
              ...
              <p className='text-muted-foreground text-xs'>
                {t('Hit rate uses all {{count}} settled requests', {
                  count: formatCompactNumber(
                    summary.cache.denominator_requests,
                    locale
                  ),
                })}
              </p>
        ...
        {summary.context && (
          <Card size='sm'>
            ...
            <CardContent className='flex flex-col gap-2'>
              ...
              <p className='text-muted-foreground text-xs'>
                {t('{{long}} long tokens · {{short}} short tokens', {
                  long: formatCompactNumber(
                    summary.context.long_input_tokens,
                    locale
                  ),
                  short: formatCompactNumber(
                    summary.context.short_input_tokens,
                    locale
                  ),
                })}
              </p>
              <p className='text-muted-foreground text-xs'>
                {t('{{coverage}} classified · {{unknown}} unclassified', {
                  coverage: new Intl.NumberFormat(locale, {
                    style: 'percent',
                    maximumFractionDigits: 1,
                  }).format(summary.context.classification_coverage),
                  unknown: formatCompactNumber(
              ...
              <p className='text-muted-foreground text-xs'>
                {t(
                  'Coverage excludes rows without a configured Context threshold.'
                )}
              </p>
              <p className='text-xs'>
                {t('Long: {{long}} · Short: {{short}}', {
        ...
        {(summary.unallocated_adjustment_quota ?? 0) > 0 && (
          <Card size='sm'>
            <CardHeader>
              <div className='flex items-center gap-2'>
                <IconBadge tone='warning' size='stat'>
                ...
                <CardTitle>{t('Unallocated Async Adjustments')}</CardTitle>
              </div>
            </CardHeader>
            <CardContent className='flex flex-col gap-2'>
              <div className='font-mono text-xl font-bold tabular-nums'>
                {formatQuota(summary.unallocated_adjustment_quota ?? 0)}
              </div>
              <p className='text-muted-foreground text-xs'>
                {t(
