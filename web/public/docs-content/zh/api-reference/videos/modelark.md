...
并可按模型能力接收图片、参考视频和参考音频。只有 Standard 支持平台托管的真人素材。
...
公开模型为 `seedance-2.0-standard-720p`、`seedance-2.0-standard-1080p`、
`seedance-2.0-value-720p`、`seedance-2.0-value-1080p` 和
`seedance-2.0-value-4k`。它们的分辨率由模型名固定，`resolution` 省略或传相同值；时长为
4～15 秒（默认 5），画幅为 `16:9`、`4:3`、`1:1`、`3:4`、`9:16`、`21:9`（默认
`16:9`）。

这五个 SKU 要求至少一个非空文本项，支持最多 9 张图片和 3 段参考音频，不支持参考视频或
`asset://`。允许文本、单首帧、首尾帧，或参考图加可选参考音频；音频不能单独使用，也不能
用于首尾帧模式。`generate_audio` 只能省略或传 `false`，回调、服务档位、草稿、水印、seed
...
模型明确支持时，普通图片或音频可以使用请求级公网 URL/Data URL；它们不会自动进入平台素材
库。需要平台复用、渠道绑定或授权治理的素材使用 `asset://ast_xxx`。平台已识别为真人的素材
...
`app_id` 作用域的 HMAC 摘要，不保存或向 Provider 发送原文。该字段不支持
不具备平台素材能力的固定分辨率 Seedance 2.0 SKU；对这些模型传入时返回
...
上述五个固定分辨率 SKU 第一阶段只支持本地列表、任务查询和内容下载：排队任务返回
