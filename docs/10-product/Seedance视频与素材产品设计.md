...
当前代码已经实现本文描述的产品边界，包括第 10.3 节的 Channel 默认素材组北向回退。默认
素材组第一批管理能力已实现：管理员可在已保存 Channel 的连通性测试下方创建或复用固定名称组，网关
保存 Channel 级内部 ID；普通素材请求的自动回退、公开文档同步和真实 Provider 验收仍未完成。每条官方或第三方线路是否可以向客户
...
该行为已经接受但尚待代码和真实 Provider 验收，完成前不得作为生产能力对外承诺。
