...
本文定义 Seedance 专用渠道、ModelArk V3 视频任务、无状态素材代理和真人认证代理的当前产品设计，回答
“管理员如何配置、客户如何使用、平台承诺什么、失败时如何表现”。具体系统组件、状态机和数据字段
由[Seedance 专用渠道与 Link 架构](../20-architecture/Seedance专用渠道与Link架构.md)负责。

当前代码已经实现本文描述的产品边界，包括第 10.3 节的 Channel 默认素材组行为：管理员创建/复用
固定名称组、北向 `asset_group_id` 可选字段与默认组回退的代码、公开元数据与本地回归均已完成；
真实 Provider 验收与生产灰度仍未完成。每条官方或第三方线路是否可以向客户
开放，仍须分别完成真实 Provider、内容、素材、账单、失败和灰度验收；渠道能够保存或单个请求成功不
等于生产发布。

## 2. 产品背景

Seedance 是高成本、长耗时的异步视频服务。官方火山、海外 BytePlus 和第三方中转商虽然都能提供
Seedance 模型，但南向接口、状态、素材能力和账号作用域并不完全相同。若把这些线路伪装成普通
...
5. 官方与第三方素材能力都通过带客户模型的无状态单资源代理提供；无素材能力继续使用请求级 URL，
   平台不建立本地资源所有权或跨 Provider 兼容结论。
6. 技术人员与管理员各自负责擅长的部分：技术人员判断兼容性，管理员使用普通字段配置和运营。
7. 在保持 NEWAPI 原生合同不变的前提下，复用现有鉴权、分组、计费、任务、日志和管理底座。
8. 统一北向协议是 Link 客户体验的第一优先级；南向 adapter 吸收 Provider 差异，客户不因 Provider 是否
   使用素材组而改变素材创建请求。
...
同一 Channel。素材边界建立后，渠道类型不可修改。管理员在当前渠道编辑界面修改 Base URL、视频/素材
协议、Project 或 Region 时，页面立即显示“本次保存将替换素材租户”和精确的新旧值；点击原保存按钮后
复用现有确认弹窗，管理员勾选风险确认后才提交。保存成功仍是原 Channel ID 和客户模型，但新的请求使用
新素材租户和新 `reuse_scope`；旧素材 ID/引用可能不可用，平台不迁移或删除旧素材。轮换视频 Key 或素材
AK/SK 且不改边界时，仍须在确认弹窗声明素材租户未变化；系统记录两类声明，但不代替管理员向 Provider
核实账号归属。

### 8.4 默认普通 AIGC 素材组

对于代码素材协议能够创建普通 AIGC 素材组的 Channel，管理员在已保存渠道的编辑页面手工创建或复用
固定名称 `aigctokenaigeneral`。系统为该 Channel 保存一个当前默认 Provider group ID；业务素材请求
不得自动创建组，存量 Channel 也不自动回填。

默认组只用于统一客户体验：客户可以直接创建普通素材，不需要先调用素材组接口。停用 Channel 保留
默认 ID，复制 Channel 不继承，删除 Channel 不删除 Provider 组。该配置不是客户素材组目录，不提供列表、
所有权或状态管理。

普通素材的组行为由代码注册的唯一策略决定：`default_fallback` 使用显式组或 Channel 默认组，`none` 不向
南向发送组字段。策略差异不得改变北向 `asset_group_id` 的可选性，也不得由客户判断。

### 8.5 Priority 与 Weight
...
改变 Provider 账号、Base URL、Region、Project、国内/海外类型或素材协议时，管理员必须执行明确的素材
租户替换确认。替换只影响保存后的新请求；旧素材不被解释、迁移或删除，在途任务继续使用创建时冻结的
旧连接和素材事实。
...
分辨率和数量等计费安全边界。合同外 Provider 私有字段明确失败，不静默删除、钳制、降级或改义；已经
属于统一北向合同的条件生效字段由 adapter 转换或忽略，不能因为某个 Provider 不使用同名字段而造成
客户请求错误。
...
平台不提供长期视频存储。生成成功后，客户应及时下载保存；控制台成功结果区显示临时下载说明。
没有明确到期时间时，不显示精确倒计时，也不承诺从创建或完成时起固定 24 小时可用。
有明确过期证据时下载入口提示内容或链接已过期；其它网络、权限或读取异常不统一解释为过期。
内容不可用不改变已确认的生成成功与计费事实，不自动退款、刷新签名或重新生成。
到期后不可用属于临时交付的生命周期；有效期内不能下载则按交付故障核查。
该提示与过期识别已完成本地实现，生产发布和真实交付验收仍须完成。

## 10. 无状态素材代理产品

### 10.1 每个模型三选一

每个 Seedance 客户模型只能发布以下一种素材能力形态：

| 形态 | 客户体验 | 管理配置 |
| --- | --- | --- |
| 官方素材能力 | 使用火山国内或 BytePlus 官方素材接口 | 对应官方素材协议和 Provider 账号作用域 |
| 第三方素材能力 | 使用中转商已经代理的素材接口 | 对应第三方素材协议，通常复用渠道 Key/Base URL |
| 无素材能力 | 仅使用请求级 HTTP/HTTPS URL 或 Data URL | 素材协议选择 `none` |

客户不在请求中选择素材能力、Region 或 Provider。模型名称和固定渠道已经确定这些事实；系统不会在官方、
第三方和无素材能力之间运行时切换。
...
协议或上游原始模型，也不建立客户 Asset/AssetGroup 表、resolver、所有权映射或列表索引。不同线路是否
接受同一 ID 由 Provider 决定；平台不迁移、探测或自动切换。Channel 默认素材组只是一条内部基础设施
配置，不改变调用方自管 opaque 素材身份的边界。
...
`asset_group_id` 是统一北向素材创建协议中的合法可选字段。普通 AIGC 素材创建遵守以下客户行为：

- 调用方提供裁剪后非空 ID 时保持原值并优先使用；
- 未传、空或空白时使用当前 Channel 默认组；
- 非空值违反已发布北向结构约束时返回 `invalid_request`，不得改用默认组；
- 默认组未配置时返回 `default_asset_group_not_configured`，业务请求不自动创建；
- Provider 拒绝显式 ID 后不回退默认组；
- 南向不使用素材组时正常创建素材，adapter 不发送组字段，客户请求不报错。

客户直接调用素材组创建而南向无此能力时仍返回既有 `unsupported_asset_operation`。固定名称
`aigctokenaigeneral` 是系统保留名称，普通调用方不能创建。真人素材继续使用 Provider 认证产生的专用
组，不使用普通默认组。

该行为的代码与公开文档已完成；真实 Provider 验收与生产灰度完成前不得作为生产能力对外承诺。
...
- 每个普通客户素材或素材组 POST 都创建一个新资源，不提供平台幂等键或自动重复检测；管理员创建或复用
  Channel 默认组是独立管理动作，不属于客户 POST；
...
- 创建前按客户模型和请求参数预扣：依赖实测 Token 的表达式必须配置预扣 Token 预算，预算是资金
  授信上界，不是 Provider 接受上限或最终扣费封顶；FunCloud V3/Synlink 纯固定或冻结条件表达式
  可不配置预算。可信终态只结算或退款一次；
- 时长、分辨率、数量等客户可控乘数先做上界校验，异常输入不能产生负费用；
- 历史任务使用创建时的价格和计费事实，不按当前价格重新计算；当前 Seedance 客户模型统一按火山
  官方刊例价换算的美元单价定价（运营口径见[计费与分组运维手册](../40-operations/01-计费与分组运维手册.md)）；
- `unknown` 不自动退款；客户退款与 Provider 潜在成本分别记录；
- Provider 实际金额未知时显示未知，不用客户 quota 或当前报价伪装 Provider 成本。

依赖实测 Token 的成功任务缺少可信用量时，视频保持成功、预扣保留并进入有限自动补查
（`awaiting_usage`），补查耗尽后转运营核实，不把预算当实测结清；明确零用量与未报告分别处理。
字段缺失、类型错误或超界的用量不参与结算，不使用 `pointConsume` 反推；`pointConsume` 仅作为可选
...
| --- | --- | --- | --- |
| 客户模型、平台 Task、客户费用 | 可见自身范围 | 可见授权管理范围 | 可见授权诊断范围 |
...
所有 Task 和平台持久化客户资源按 `user_id + app_id` 隔离。素材代理不持久化客户 Asset/AssetGroup，
opaque ID 由受信应用管理；Channel 默认组 ID 是渠道级基础设施配置，不是客户资源。缓存和前端状态不能
成为任务、资金、权限或默认组 ID 的唯一事实源。
...
| 客户提交合同外 Provider 私有字段或请求不支持操作 | 返回明确的不支持，不静默删字段或伪造成功 |
| Provider 明确拒绝创建 | 任务创建失败，按已确认事实处理预扣 |
| Provider 创建结果不明 | 进入 `unknown`，不重发、不换渠道、不自动退款 |
| 单次任务查询不可采信 | 客户看到「状态待核对」标签、提醒色与核对原因说明；保持原业务状态并继续查询，提示暂勿重复提交。取得可信终态后说明随状态更新并清除活动告警，不渲染为业务失败 |
| 当前模型不支持素材或具体操作 | 返回 `unsupported_asset_operation`，不尝试其它 Provider |
| 普通素材需要默认组但 Channel 尚未配置 | 返回 `default_asset_group_not_configured`，提示管理员先完成渠道配置 |
| 南向不使用素材组但请求携带 `asset_group_id` | 正常创建素材，adapter 不发送组字段 |
