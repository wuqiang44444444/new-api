...
Provider 模型。当前渠道只使用代码协议 `feicai_videos_v1` 和无 `-feicai` 后缀的当前模型，不保留
...
- 飞彩历史版本 Channel 已移除，当前环境不存在旧 Ability；
...
2. 配置正式可验证 HTTPS Base URL，建立 VIP 五模型和性价比五模型两个单凭据 Channel，并登记 10 个精确 model mapping。
3. 导出客户模型、唯一 Channel、Provider 模型、代码协议、价格和 Provider exposure 快照。
4. 每个用例只发送一次创建请求；记录平台 `request_id`，不得用客户幂等键重放。
5. 准备可验证的 HTTP/HTTPS 图片、图片 Data URL、HTTPS 音频和 HTTPS 视频；记录 MIME、大小、有效期和所有权，不保存完整源 URL 到共享证据。
6. 准备余额前后快照、usage/subscription 和 NEWAPI 结算日志的只读核对方式；不把账户趋势猜成单任务成本。
7. 记录两个五模型 Channel 的停流方式；回滚只阻止新流量，不能中断已创建 Task 按冻结 Channel 与协议的轮询和结算。
...
| ---: | --- | --- | --- | --- |
| 1 | `seedance-2.0-mini-720p` | 4 秒、15 秒；720p；六种登记画幅；图片/音频允许组合 | 3/16 秒；未登记画幅；10 图/4 音频/任意视频 | 按秒、模型独立单价 |
| 2 | `seedance-2.0-sd2-720p` | 1 图+11 秒、9 图+15 秒；16:9、9:16 | 无图；10 图；10/16 秒；任意音频/视频；其它画幅；省略 duration | 按秒；失败是否扣费 |
| 3 | `seedance-2.0-fast-720p` | 4/15 秒；720p；六种登记画幅；图片/音频 | 越界时长、媒体和未登记画幅 | 按秒；记录时延但不宣称 SLA |
| 4 | `seedance-2.0-value-720p` | 六种登记画幅；4/15 秒；内容下载 | 未登记画幅、媒体越界 | 按秒；不继承历史结果 |
| 5 | `seedance-2.0-standard-720p` | 独立执行同类场景 | 不接受 value 模型证据代替 | 独立单价、独立 Provider 任务 |
| 6 | `seedance-2.0-value-1080p` | 1080p；六种登记画幅；4/15 秒；Range | 720p/4K resolution、未登记画幅 | 按秒、高分辨率成本 |
| 7 | `seedance-2.0-standard-1080p` | 独立执行 1080p 场景 | 不接受 value size/账单代替 | 独立单价 |
| 8 | `seedance-2.0-value-4k` | 4K；六种登记画幅；最大时长；Range | 非 4K、未登记画幅、额度不足 | quota 上界、内容大小 |
...
除 SD2 只接受 `16:9`、`9:16` 外，其余九个模型接受代码登记的 `21:9`、`16:9`、`4:3`、`1:1`、
`3:4`、`9:16`。南向发送 `ratio`，不发送 `size`；720p/1080p/4K 输出档位由精确 Provider 模型决定。
未登记画幅保持拒绝，直到形成新证据并重新审批。
...
7. 核对冻结 customer model、Channel、Provider model、`feicai_videos_v1` 协议和 billing probe。
8. 核对客户预扣、终态结算/退款和 Provider 账户用量趋势，保持客户费用与 Provider 未知成本分离。
...
- 当前没有可用的 Provider 任务列表恢复源；unknown 只按冻结事实人工核实，不建立自动 CAS 恢复。
- 当前协议的任何 terminal rejection 必须独立验证精确 HTTP status 与 error.code；不能沿用历史证据。

## 7. 素材与内容专项

- 图片 HTTP、HTTPS 与允许的图片 Data URL 分别验证；音频、视频只使用 HTTPS。
- 验证带 userinfo URL、本地路径、相对路径、错误 Data MIME、非法 Base64 和过期 URL 均失败关闭或得到可信 Provider 拒绝。
- 验证直接媒体可用；`asset://<opaque-id>` 原样进入飞彩 Provider，由上游明确接受或拒绝，平台不查询
  素材、识别来源或 fallback。素材 CRUD 元数据仍为不支持。
...
- 目标生产 Base URL 的 `/v1/tasks` 已实测 404，不作为任务级对账源，也不按相近时间猜测任务成本；
- usage/subscription 只作为账户趋势，其单位与货币必须在生产验收中重新确认；
...
- 精确 customer model、唯一 Channel、Provider model 和 `feicai_videos_v1` 协议一致；
- 上线清单中的 duration、resolution、size、媒体 min/max/roles 与黑盒一致；
- 成功、可信失败、unknown、单次创建、轮询和内容下载均通过；
- 客户账单、Provider 证据和 exposure 可分别核对；
- 无敏感信息泄漏；
- 所属五模型 Channel 的停流和回滚边界明确；若要求逐模型灰度，启用前先收窄 Channel 模型清单。

某一模型通过只授权该模型及已验证组合，不授权同族、同分辨率或同报价模型。

## 10. 停流与回滚

出现以下任一情况立即禁用对应的五模型飞彩 Channel；若已提前配置逐模型灰度清单，则移除对应客户模型：
