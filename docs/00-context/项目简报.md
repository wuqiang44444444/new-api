...
图片中的核心内容需要纳入 LinkMetaX 首页与用户文档，但只参考信息结构，不照搬视觉样式。核心目标是让用户理解：**获得 API Key 后，只需在常用工具中替换 Base URL 与 API Key，即可通过一个入口调用平台支持的多个模型。**
...
无需逐个对接不同模型供应商。复制 LinkMetaX API 地址和 API Key，按照工具说明完成配置，即可在常用开发工具与 AI 应用中调用平台已开放的模型。
...
域名和路径必须由当前部署配置动态生成，不能在产品文案或代码示例中写死 `tokenai.page`、`api.linkmetax.com` 等地址。
...
```bash
export ANTHROPIC_BASE_URL="<LINKMETAX_ANTHROPIC_BASE_URL>"
...
```text
API Key: sk-your-key
Base URL: <LINKMETAX_OPENAI_BASE_URL>/v1
```
...
```json
{
  "baseURL": "<LINKMETAX_OPENAI_BASE_URL>/v1"
}
```
...
```yaml
models:
  - name: LinkMetaX Model
    provider: openai
    model: <available-model-id>
    apiBase: <LINKMETAX_OPENAI_BASE_URL>/v1
    apiKey: sk-your-key
```
