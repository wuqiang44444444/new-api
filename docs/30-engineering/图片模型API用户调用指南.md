...
三个目标模型使用相同的客户 API 合同路径和字段；切换模型时通常只需修改 `model`：
...
如果 Provider 返回可信完整的终态结果，但结果确定违反公开交付合同，例如去重后图片数超过
请求 `n`，当前版本会把任务投影为 `failed/image_generation_failed`。客户端应停止轮询；不要
把该错误解释为结果数量仍可能变化。
...
同步图片成功后不会创建 Task，也不会保存响应用于幂等回放。平台会完成计费并释放临时幂等
claim。

因此：

- 客户端收到 HTTP `200` 后，不要再次提交同一业务请求。
- 如果响应在到达客户端前断网，以同一 Key 重试仍可能创建新图片并再次计费。
- `Idempotency-Key` 不能被理解为同步图片的 exactly-once 或结果存储保证。
- HTTP 客户端不要对图片创建 POST开启无条件自动重试。

业务上必须避免重复图片时，应在客户端保存业务请求状态、平台请求 ID、返回 URL 或异步 Task
ID，并让人工或补偿流程处理同步结果不确定的情况。
...
错误消息只描述 Link 公开字段，不应出现供应商密钥、内部渠道 ID、上游模型 ID或上游任务 ID。
