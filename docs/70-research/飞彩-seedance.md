...
截至 2026-08-02，本文只能证明飞彩 PI 与 AZHW 模型声明支持真人图片作为视频请求的参考素材。
现有资料没有提供平台托管真人素材所需的完整控制面合同，包括：

- 真人 H5 认证会话的创建、查询、过期和重试；
- 认证结果与稳定人物/素材组标识；
- 真人图片的入库、状态查询、视频引用值和账号作用域；
- 撤回、删除、清理和创建结果未知时的对账语义。

因此，飞彩真人素材能力当前保持未接入，不对外宣传、不进入真人灰度，也不将普通 HTTP/HTTPS URL 或
Data URL 的技术透传视为真人素材接入。只有飞彩补齐上述书面 API 合同并完成真实渠道验证后，才重新评估接入。
...
| `seedance2.0-sd2-feicai` | 11–15 秒 | 固定 720p | `16:9`、`9:16` | 必须 1–9 张 | 不支持 | 不支持 | 不支持 |
...
```json
{
  "audio_urls": [
    ...
    "https://cdn.example.com/ref/voice.wav"
  ]
}
```
...
| `image` | string[] | 是 | 1–9 个公网 HTTP/HTTPS 图片 URL |
...
```json
{
  "image": [
    ...
    "https://cdn.example.com/ref/prop.png"
  ]
}
```

图片顺序从 1 开始。提示词支持以下引用写法：
...
```text
让 @图1 中的人物进入 @图2 的场景，拿起 @图3 中的道具，
人物外观、服装和道具细节保持一致。
```

如果某张已上传图片没有在提示词中明确出现，临时会自动补齐该图片引用。

不要在同一次请求里混用 `image`、`images`、`image_url` 和 `reference_image_urls`，否则相同 URL 可能被重复计入图片数量。
...
curl -X POST "https://feicai123.top/v1/videos" \
  ...
  -d '{
    "model": "seedance2.0-sd2-feicai",
    "prompt": "让 @图1 中的人物走进 @图2 的街道，保持人物外观、服装和场景细节一致，镜头缓慢跟随。",
    "image": [
      "https://cdn.example.com/ref/character.png",
      "https://cdn.example.com/ref/street.png"
    ],
    "duration": 15,
    "resolution": "720p",
    "ratio": "16:9",
