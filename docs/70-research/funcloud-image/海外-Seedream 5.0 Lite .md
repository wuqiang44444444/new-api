Overview
Seedream 5.0 Lite Image Generationendpoint, supported Text-to-Image (text-to-image) and Image-to-Image (image-to-image)Two modes.
...
curl -X POST "https://api.apiverse.ai/api/v2/open/aigc/seedream-5.0-lite" \
...
curl -X GET "https://api.apiverse.ai/api/v2/open/aigc/task_abc123" \
 -H "Authorization: Bearer your_auth_token_here"
Endpoints
1. create Seedream 5.0 Lite Image Generationtask
POST /api/v2/open/aigc/seedream-5.0-lite

Create a Seedream 5.0 Lite Image Generationtask.
...
imageUrls	string[]	Condition	Reference image URL (i2iwhen Required, up to 14 images, JPEG/PNG/WebP, each maximum 10MB)
aspectRatio	string	No	Aspect ratio:1:1(default) / 4:3 / 3:4 / 16:9 / 9:16 / 2:3 / 3:2 / 21:9
quality	string	No	output quality:basic(2K,default) / high(3K)
...
{
 "code": 40001,
 "msg": "Insufficient balance: first 0.0100, 0.0413",
 "data": null
}
2. Query Task Status
...
Text-to-Image(basic/2K):Usually 5 ~ 15 seconds
Text-to-Image(high/3K):Usually 10 ~ 30 seconds
Image-to-Image(basic/2K):Usually 5 ~ 15 seconds
Image-to-Image(high/3K):Usually 10 ~ 30 seconds
