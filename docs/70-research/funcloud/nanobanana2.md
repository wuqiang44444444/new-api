...
概述
Nano Banana 2 图片生成接口，支持文生图（text-to-image）和图生图编辑（image-to-image），支持多分辨率输出（1K/2K/4K），最多可提供14张参考图片进行图生图编辑，支持更丰富的宽高比选项。
...
imageUrls	string[]	否	参考图片 URL（最多14张，JPEG/PNG/WebP，每张最大30MB）
...
- 支持最多14张参考图片，每张最大30MB，支持 JPEG/PNG/WebP 格式
请求示例
基础文生图：
