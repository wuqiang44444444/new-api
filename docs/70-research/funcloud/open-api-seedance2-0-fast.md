...
完整素材链路见：Seedance 2.0 素材库 API 对接文档。
...
| 参数 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `content` | array | 是 | 内容数组，详见下方 content 内容项 |
...
| `returnLastFrame` | bool | 否 | 是否返回视频尾帧图片 URL |
...
> **说明：**
> - content 数组中必须包含至少一个 `text` 类型的内容项作为提示词。
> - 在提示词中通过 `图片1`、`图片2`、`视频1`、`音频1` 引用对应位置的参考素材。
> - 真人素材必须先走素材认证链路入库；虚拟素材先创建素材组再上传。生成时使用素材接口返回的 `assetUrl`（详见素材库 API 对接文档）。
