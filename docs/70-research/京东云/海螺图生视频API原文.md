...
retrieved-at: 2026-09-23T15:42:32+00:00
source-url: https://docs.jdcloud.com/cn/jdaip/hailuo-image-to-video
source-content-sha256: a940502df2b4ed41302e0e4886a6d26825fdf8542205320729f70f640309499b
---
...
- 官方来源：[京东云 JoyBuilder 模型开发平台 2.0](https://docs.jdcloud.com/cn/jdaip/hailuo-image-to-video)。
...
| **字段** | **二级字段** | **三级** | **类型** | **必填** | **含义** |
| --- | ---- | --- | --- | --- | --- |
| model | - |  | string | 是 | 模型调用值，可选：MiniMax-Hailuo-2.3 |
| content |  |  | array[object] | 是 |  |
| @rows=4: | type |  | string | 是 | 内容类型<br>text:相当于提示词文本类型<br>image\_url:图片类型 |
| text |  | string | 否 | 提示词文本 |
| @rows=2:image\_url |  | object | 否 | 图片类型 |
| url | string | 是 | 图片地址 |
| parameters |  |  | object | 否 |  |
...
> image\_url需要注意：
> 注1：支持公网 URL 或 Base64 编码的 [Data URL](https://developer.mozilla.org/en-US/docs/Web/URI/Reference/Schemes/data)
> 请注意，base64 decode之后的字节长度需要小于20M，且编码必须包含适当的内容类型字符串，例如：
> data:image/png;base64,{base64\_encode};
> 注2：格式：JPG, JPEG, PNG
> 注3：尺寸：短边像素大于 300px，长宽比在 2:5 和 5:2 之间
