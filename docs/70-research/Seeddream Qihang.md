...
客户端只使用 NEWAPI 客户 API 合同，不感知启航的接口差异：
...
客户端统一模型名为 `seedream-5`。启航的上游模型名同样是 `seedream-5`，不需要模型名转换。
...
因此，启航是两个上游中更接近 NEWAPI 原生协议的一方。生成接口主要可以通过“高级自定义渠道 + 原生透传 + 参数覆盖”配置完成；改图接口只有在客户端请求严格限制为 `model + prompt + image` 时才能原生透传。若要接受并过滤更多 NEWAPI 可选字段，应增加启航专用的轻量 Multipart 过滤转换器。
...
| `size` | 默认 `2K`；允许 `2K`、`3K` | 与 Moxing 的明确值域对齐；启航侧做 Provider 尺寸转换 |
...
客户端保持 NEWAPI 已有的 OpenAI 风格 Multipart 接口：
...
| `quality/style/watermark/...` | 未声明 | 客户端验证阶段拒绝 |
...
Moxing 和现有转换器要求 `2K/3K`。因此客户端不能把供应商尺寸直接暴露给客户端。
...
| `size` | 未声明 | 第一阶段客户端禁止显式传入 |
| `mask` | 未声明 | 客户端直接返回 `400` |
...
生成接口当前可用的目标配置如下。改图路由只有在 Seedream 5 能力实测通过、且客户端严格限制为最小 Multipart 字段时才可以加入：
...
- 不修改 `dto.ImageRequest` 和现有客户端路由定义。
