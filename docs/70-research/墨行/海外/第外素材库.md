...
- **接口根路径**: `https://tokensave.pro/assets`
- **鉴权**: `Authorization: Bearer sk-xxxx`

## 概览与认证

海外素材库 API 使用平台 API Key 鉴权，平台会根据 Key 识别用户，并只允许访问当前用户绑定的素材。
...
- 素材访问权限绑定到 API Key 对应用户，不能跨用户查询或引用。
...
| 接口 | 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- | --- |
| 创建素材 | `url` | string | 是 | 素材地址。`asset_type=Image` 时支持 HTTP(S) URL、`data:image/...;base64,...` 或裸 base64；Video/Audio 仍需 HTTP(S) URL。 |
| 创建素材 | `name` | string | 否 | 素材名称，最长 64 字符。 |
| 创建素材 | `asset_type` | string | 是 | 素材类型，支持 `Image` / `Video` / `Audio`。 |
| 查询列表 | `statuses` | string[] | 否 | 状态过滤，支持 `Active` / `Pending` / `Processing` / `Failed`。 |
| 查询列表 | `name` | string | 否 | 按素材名称模糊搜索。 |
| 查询列表 | `page_number` | integer | 否 | 页码，默认 1。 |
| 查询列表 | `page_size` | integer | 否 | 每页数量，默认 20，最大 100。 |
| 更新素材 | `name` | string | 是 | 更新后的素材名称，最长 64 字符。 |
...
```bash
curl -X POST 'https://tokensave.pro/assets' \
...
```bash
curl -X POST 'https://tokensave.pro/assets/list' \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer sk-xxxx' \
  -d '{
    "statuses": ["Active"],
...
```bash
curl 'https://tokensave.pro/assets/ea287e6e-13fb-4af4-a953-9e7cbb883ee5' \
  -H 'Authorization: Bearer sk-xxxx'
```
...
```bash
curl -X POST 'https://tokensave.pro/assets/ea287e6e-13fb-4af4-a953-9e7cbb883ee5/update' \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer sk-xxxx' \
...
```bash
curl -X POST 'https://tokensave.pro/assets/ea287e6e-13fb-4af4-a953-9e7cbb883ee5/delete' \
  -H 'Authorization: Bearer sk-xxxx'
```
...
此时素材通常仍处于 `Pending` 或 `Processing`。
...
素材状态为 `Active` 后，可在视频请求中引用。V2 媒体任务（如 sd2.0）使用 `image` / `reference_images` 等字段；海外官Key 模型 `dreamina-seedance-2-0-260128` 请走海外官Key视频，在官方 `content[]` 里使用 `asset://{upstream_id}`。

```bash
curl -X POST 'https://tokensave.pro/v1/media/generations' \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer sk-xxxx' \
  -d '{
...
```json
{
  "detail": "素材不属于当前用户: asset://asset-20260401184821-8tkgk"
}
```
