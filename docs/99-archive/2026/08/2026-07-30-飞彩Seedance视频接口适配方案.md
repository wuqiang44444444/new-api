...
source-path: docs/80-dev/2026-07-30-飞彩Seedance视频接口适配方案.md
superseded-by: "docs/20-architecture/视频上游接入与异步任务架构.md; docs/30-engineering/视频模型API用户调用指南.md; docs/40-operations/02-视频与素材渠道运维手册.md"
---
...
> 飞彩仍只实现本文的 ModelArk Link SKU，不得进入恢复后的原生 `/v1/videos` 候选集。
...
本方案不让飞彩或其 Link SKU使用 `POST /v1/videos`，也不让客户端使用飞彩私有字段。飞彩客户端继续调用：
