...
>
> 当前配置入口与操作步骤见 [02 视频与素材渠道运维手册](../40-operations/02-视频与素材渠道运维手册.md)；表达式完整语法见 [表达式计费系统设计](../../pkg/billingexpr/expr.md)；任务快照、usage 回填与结算边界见 [视频上游接入与异步任务架构](../20-architecture/视频上游接入与异步任务架构.md)。
...
渠道接入（地址、Key、路径后缀）见 [02 视频与素材渠道运维手册](../40-operations/02-视频与素材渠道运维手册.md)。表达式配在**公开模型名**上（不是映射后的上游名），计费类型选「表达式计费」，把上面的表达式贴入。
