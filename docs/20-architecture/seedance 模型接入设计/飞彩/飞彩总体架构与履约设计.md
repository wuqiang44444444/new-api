...
本文定义飞彩 10 个 Seedance 模型共享的合同边界、单轨 v2 实现、请求转换、异步 Task、Link 资源、
计费探针和内容代理。逐模型差异见[全模型 SKU 与计费设计](飞彩全模型SKU与计费设计.md)。

目标只使用 `feicai.seedance-videos/v2`：

- 不保留飞彩 v1 渠道作为等价候选；
- 不建设 v1/v2 多版本 registry、双 parser、双内容代理或运行时 fallback；
- v2 落地时删除飞彩 v1 implementation、media-arrays adapter v1 专用分支、v1 capability 版本和对应测试；
- 其它 Provider 仍可继续使用它们各自的 v1 implementation，不能删除全局 `LinkImplementationVersionV1`；
- 删除 v1 前必须证明所有部署环境不存在仍需履约或访问的飞彩 v1 事实。

当前代码中的 v1 只是旧窄实现事实，不是目标架构组成部分。它只履约两个 720p SKU，采用全局两元 size 表、
media-arrays adapter v1、图片/音频数组，并明确拒绝视频输入。
...
| Link implementation | 唯一目标 `feicai.seedance-videos/v2` | 飞彩路径、adapter、素材方式和 execution bindings |
...
customer_model
  ...
  -> provider_model
  -> feicai.seedance-videos/v2 execution binding
  -> Link SKU
```
...
profile 表达协议族，implementation version 和 adapter revision 表达已冻结执行语义。v2 content hash 至少覆盖：
...
当前 implementation registry 以 ID 为唯一键，正好支持本设计的单轨替换。目标不增加同 ID 多版本共存结构；
代码中只登记飞彩 v2。Channel 的 `LinkImplementationRef`、Task 快照和内容代理只接受 v2。
...
5. exposure 与 implementation v2 资格过滤
...
| 固定分辨率 + `ratio` | `size` | 查 v2 逐模型 size registry |
...
只存在 `720p:16:9 -> 1280x720` 与 `720p:9:16 -> 720x1280`。当前
`ResolveVideoSize(resolution, ratio)` 同时被 create converter 和 billing probe 调用，不具备 implementation 或
provider model 维度。
...
create converter 与 billing probe 必须接收同一个冻结实现身份和映射后的 Provider 模型，调用同一 resolver。
不得保留旧两元 resolver 作为 fallback；未登记组合在预扣前返回能力不支持。
...
当前代码中的 `HTTP 403 + feicai_account_required` 只为飞彩 implementation v1 登记，v2 不继承。
删除 v1 分支后，v2 在取得“精确 status + error.code 能证明未创建”的独立证据前，所有非 2xx 都进入
`unknown`。证据成立后只为 `feicai.seedance-videos/v2` 增加精确组合及回归，不扩大到整个状态码或 message。
...
`GET /v1/tasks` 只可作为 create unknown 或已知 Task 的对账源，使用另一套大写状态：
...
两套状态解析器必须独立，不能先转小写后复用主轮询映射。只有能以冻结 request ID、幂等键或其它唯一键
精确关联，并通过 CAS 幂等恢复时，任务列表才可改变 attempt/Task；不得使用“最近一条相似任务”。

无效 JSON、ID 不匹配、未知状态、完成无 URL 或结果 URL 不满足安全规则都属于不可采信响应。

## 10. Link 资源与多媒体输入

飞彩 v2 复用 Link Resolver 的 `source_url` 模式：
...
asset://ast_*
  ...
  -> 全部素材的候选渠道交集
  -> 选中飞彩 v2
  -> 发送前 TTL 与 implementation v2 二次校验
  -> 解密 HTTPS source URL
...
飞彩资料所称“支持真人内容”不等于平台 `real_person` 合同。飞彩 v2 只注册 `asset_kind=general`；
...
new-api -> 读取 Task 冻结的飞彩 v2 Base URL、Key、implementation 和结果 URL
...
飞彩 v2 只有在下列代码权威同时一致时才是完整实现：

| 权威面 | v2 必须表达的事实 |
| --- | --- |
| SKU capability | 10 个 SKU、固定分辨率、时长/default、媒体 min/max/roles、billing mode、v2 hash |
| Link implementation | 唯一 `feicai.seedance-videos/v2`、10 bindings、v2 adapter、素材上限与 content hash |
...
| durable create | v2 确定拒绝独立登记；其它发送后异常 unknown |
| polling/reconcile | 小写主轮询与大写任务列表分别归一化，均使用冻结顶层 id |
| billing | 按秒/按次分离，checked quota、saturation 审计、exposure 闭环 |
| content | 只接受冻结 v2、同源 HTTPS、冻结 Key、拒绝重定向 |
| regression | 移除 v1 fixture，10 模型与 v2 单轨回归全部通过 |
...
2. 飞彩运行时只有 implementation v2；不存在 v1/v2 并行或 fallback。
...
9. Task、attempt、计费、素材和内容代理冻结 implementation v2 与 publication 事实。
