...
当前只使用 `feicai.seedance-videos@v2`：
...
当前 registry 只登记 `feicai.seedance-videos@v2` 下 Mini 720p、Standard 720p 和 Standard 1080p
三个 Provider 模型的 `16:9` 精确条目，证据版本为 `feicai-prod-2026-08-05-r1`。旧
`720p:16:9` 与 `720p:9:16` 两元记录没有作为 fallback 保留。
...
已移除的飞彩 implementation v1 曾登记 `HTTP 403 + feicai_account_required`，v2 不继承该规则。
v2 在取得“精确 status + error.code 能证明未创建”的独立证据前，所有非 2xx 都进入
...
飞彩 v2 复用 Resolver 的 `source_url` 模式：
