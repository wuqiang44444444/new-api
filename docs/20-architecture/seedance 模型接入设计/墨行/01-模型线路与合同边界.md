...
本文定义墨行当前两个 Seedance 模型、历史协议线路和 Link implementation 如何分层。两个当前
V2 Provider 模型均属于 Moxing 的 TokenSave 模型体系，对外必须按 `seedance-2-0-oversea` 与
`doubao-seedance-2-0-260128` 两个 Link SKU 的合同特征区分，不得以历史 implementation ID 前缀作为
Provider 或方案名。

当前代码注册的方案展示名为 `tokensave.<Link SKU>`：Provider 只显示一次 Moxing，`tokensave`
表示两个 Provider 模型的共同模型体系，Link SKU 负责表达客户合同差异。
...
| --- | --- | --- | --- | --- |
| `seedance-2-0-oversea` Link 方案 | `seedance-2-0-oversea` | `third_party_relay` | `relay_assets` | 当前模型；按量计费 |
| `doubao-seedance-2-0-260128` Link 方案 | `doubao-seedance-2-0-260128` | `third_party_relay` | `relay_assets` | 当前模型；按秒计费 |
...
`doubao-seedance-2-0-260128` 与 `seedance-2-0-oversea` 同属 Moxing Provider 的 TokenSave 模型体系，但
对应不同 Link SKU 和独立 V2 implementation。“同一 Provider 模型体系”不等于“同一 Provider
implementation”：前者不能配置为 Ark 资产线路，也不能因路径相似而复用
`moxing.seedance-media-task` 的身份、凭据、价格、usage 或 execution binding。
...
```text
选择精确的 `seedance-2-0-oversea` 或 `doubao-seedance-2-0-260128` Link 方案
...
1. 两个当前 Link SKU、Ark 与 JoyCreator 是四条不同履约线路，路径相似不产生等价性。
