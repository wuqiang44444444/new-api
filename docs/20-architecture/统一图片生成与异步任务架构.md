...
ADR-0011 已接受但发送前 durable attempt、`prepared/sending/unknown`、到期 exposure 和
`reconciliation_required` 尚需按本章目标时序实现。现有“收到任务信封后再持久化”的代码不能
视为已经覆盖 POST 发送与 task ID 落库之间的崩溃窗口。
...
- [ADR-0006：图片异步任务共享 Task 持久化与客户端恢复](decisions/0006-media-image-shared-task-persistence.md)
