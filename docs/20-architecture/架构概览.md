...
new-api 是统一 AI API 网关。NEWAPI 已原生支持的渠道和模型必须保持其原有合同、relay 与计费逻辑。Link 合同是由项目负责人明确新增、通过本地代码变更实现并显式注册的扩展合同；鉴权、路由、计费、异步任务和审计仍优先复用平台公共能力。
...
Link 合同是显式代码扩展与注册的封闭集合，不是全站路由的总分类。是否属于 Link 只看代码注册事实，不通过 Ability、候选渠道或运行时探测判定。已被 NEWAPI 原生支持的文生文等模型不进入以下链路。
...
- 视频在 Link 合同中分别采用 Seedance、Kling、即梦官方协议，并按公开模型/SKU 稳定发布；
- 素材库使用平台统一控制面合同，Provider Action、签名和上游资源 ID 不向客户泄漏；
- 同一 Link 公开 SKU 只能进入合同等价的渠道，Link 合同不能反向限定 Provider 只能是官方渠道；该等价门不得介入 NEWAPI 原生 relay 选渠；
- 同一公开 SKU 可由多个能力等价的已注册实现服务；渠道显式绑定不可变实现 ID/version，profile
  只描述协议适配形状，实现身份不进入公开 capability hash；
...
1. Link 合同只由获批的本地代码扩展和显式注册建立；配置、Ability、候选渠道和运行时探测均不能创建或推断 Link 合同。
2. 渠道适配协议是 Link 合同的实现子合同；它与客户可见语义解耦，不得改变同一 Link SKU 对客户的承诺。
3. 创建请求在一次选渠上下文中完成模型映射、单 Key 选择、调用和执行快照。
4. 在途任务依赖冻结快照继续轮询、投影和结算；历史兼容回退必须显式且可测试。
5. 计费乘数先校验，额度换算使用统一饱和函数；预扣、结算、退款和补偿均幂等可审计。
6. 渠道凭据、任务私有数据、素材上游 ID、签名 URL 和真人认证秘密不进入普通用户响应或日志。
7. Provider 不确定结果保留可恢复状态，不能伪装成功或盲目重复创建。

## 6. 专题架构

- [Link 合同架构](Link合同架构.md)：图片、视频和素材库共同遵循的产品合同、SKU、选渠与适配边界。
