...
本文描述已经接受、尚待完整实施和生产验收的目标架构。当前代码仍可能包含旧的 Link publication、
SKU、implementation、execution binding、通用 AssetBinding 和平台真人授权实现；这些旧实现不能再
作为新功能的设计依据。
...
- 两类能力可以共享鉴权、Token、Group、Ability、Channel、计费、Task 和日志底座，但不得互相推断
  或把原生入口包装成 Link；
...
| 管理控制面 | 用户、Token、Group、Channel、Ability、模型映射、价格和协议选择 | 主数据库 |
...
| 事实 | 权威来源 | 说明 |
| --- | --- | --- |
| 新请求是否可用 | Token、Group、Ability、Channel 状态 | 配置只影响之后的新请求 |
