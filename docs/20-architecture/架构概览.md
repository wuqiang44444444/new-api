...
和代码 adapter，直接返回 Provider opaque 素材、素材组或认证会话 ID；平台不提供列表，不持久化
Asset/AssetGroup、所有权、状态或作用域映射。
...
8. 素材控制面不建立 Asset/AssetGroup、resolver、列表或本地作用域校验；Provider opaque ID 由调用方管理。
...
- [计费表达式与协议探针架构](计费表达式与协议探针架构.md)
- [Link 图片服务合同与异步任务架构](Link图片服务合同与异步任务架构.md)
- [用户模型合同定价架构](用户模型合同定价架构.md)
