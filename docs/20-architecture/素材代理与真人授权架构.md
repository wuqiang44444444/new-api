...
素材能力是 new-api 的控制面代理，不是对象存储服务。客户提交 Provider 可访问的 HTTPS URL 后，平台创建稳定 `ast_*`，将源 URL 以 Asset 作用域加密保存，并可按明确的 Link 实现物化一个或多个上游 binding。执行时由唯一 Resolver 在 active binding 和可用 `source_url` 之间解析，不把 Provider 身份暴露给客户。

[Link 资源虚拟素材库架构](Link资源虚拟素材库架构.md) 是供应商中立的产品合同；本文说明当前素材代理、上游 binding 与真人授权生命周期的实现边界。

两篇文档的分工是：前者定义客户看到的 `ast_*`、可用性和解析语义；本文定义控制面对象如何创建、
轮询、删除、认证和撤回，不重复定义图片或视频请求合同。
...
- 平台不对源 URL 执行 GET/HEAD，不保存素材二进制；完整源 URL 只以 `asset-source:<public_id>` 作用域加密密文保存；
- 创建时 `model`/`target` 可选；平台可先建立只有 source 的逻辑 Asset，再按拟使用的代码注册实现创建 binding；
- 同一 Asset 可拥有 0..N 个 binding，但 binding 只能在冻结的渠道、账号、profile 和 Link implementation ID/version 下消费；
...
早期 Azure 二进制托管、单 binding 和 FunCloud multipart 流式方案从未发布，已从有效 ADR 集合
移除，不存在存量迁移或兼容义务。当前唯一素材实现身份与 source/binding 决策由 ADR-0015 承接；
历史方案只通过 Git 追溯，不能用于扩展新实现。
...
- `AssetSource` 封装、作用域认证加密与查询时 Asset 聚合状态；
- 代码所有的 Link implementation 注册表，以及 binding 的实现 ID/version、策略与凭据围栏；
- `asset://` 的 binding/source 双模式 Resolver，选渠道和每次 Provider 尝试前都重新校验；
- FunCloud v2 通过 JSON `content` 消费 source URL；旧 H5/raw upload/multipart、素材列表与专用 attempt 链已删除。

旧 target/profile 别名只用于迁移兼容，不属于新 Link 合同。其调用量、退出日期以及全部 Provider 的真实生命周期和三数据库灰度仍需持续验收。

代码完成不等于生产已发布。FunCloud Standard/Fast、Provider 托管素材与外部数据库仍须按真实
Provider 和全渠道手册验收。图片 Link 资源当前尚未发布：三个图片 capability 都是
`supports_link_assets=false`，图片 Router 也未接入 Resolver。FunCloud `realPersonMode` 在获得
书面授权/撤回合同前不是公开真人能力。
...
用户
 ...
 │   ├─ AssetSource（作用域加密 URL + expires_at）
 │   └─ 0..N AssetBinding
 │       ├─ implementation ID/version + policy
 │       ├─ upstream profile + channel + credential fingerprint
...
| Asset Service | 用户隔离、幂等、AssetSource、聚合状态和生命周期编排 | 不保存素材二进制 |
| Link 实现注册与 Resolver | 按公开 SKU、实现 ID/version、分组、binding/source 能力和凭据作用域选择并改写引用 | 不根据域名、模型名或 Key 格式猜协议 |
...
`Asset` 表达用户拥有的平台逻辑资源；`AssetSource` 是它的不可变加密执行源，`AssetBinding` 表达它在某个上游账号和 Link 实现版本下可用。客户始终只使用 `ast_*`；更换源 URL 必须通过显式迁移创建新 `ast_*`，不允许在原资源上替换媒体身份。
...
sequenceDiagram
    ...
    participant D as Link 实现注册与分组选择
    participant U as 上游素材适配器
    participant DB as 数据库与 Operation Job

    C->>A: POST /v1/assets + HTTPS URL
    A->>A: 校验用户/App、URL、TTL、类型、授权与幂等
    A->>DB: 创建 Asset/AssetSource/幂等记录
    opt 请求指定可物化的 model/target
        A->>D: 解析代码注册的 implementation ID/version
        D-->>A: profile、channel、policy、credential fingerprint
        A->>DB: 创建 Binding/所有权 claim/超时 watchdog
        A->>U: 创建上游 group（如需要）和素材
...
普通素材可以不指定 `model`/`target`，此时先创建只有 AssetSource 的供应商中立逻辑资源。如果指定目标，只能选择一个：
...
Provider profile 与业务实现的配对由代码注册表显式所有；下表是当前上游托管 binding 的适配形状，不是实现身份：
...
任何启用上游托管素材 profile 的视频渠道都必须是 DoubaoVideo 单 Key 渠道，并配置正数 `asset_min_url_ttl_seconds`。只支持 `source_url` 的图片或视频实现也必须在注册表中明确声明 Link 解析能力和最小 TTL。系统不根据 Base URL、模型名或 Key 格式推断 profile 或 implementation。
...
上述 TTL 合同同时用于当前 `source_url` 任务执行：
...
`Asset`、`AssetSource`、幂等记录在同一数据库事务中创建。需要上游物化时，`AssetBinding`、实现版本快照、所有权 claim 和 watchdog 使用同一业务事实提交。上游对象在 Provider 账号作用域只能被一个本地 Asset 认领；冲突时 fail closed。删除或撤回与上游创建并发时，晚到结果只能补记上游 ID 并进入清理，不能把本地资源复活为 ready。
...
3. 从代码注册表解析公开 SKU 的候选 implementation ID/version 和 Link 解析策略；
4. 为每个素材生成可用路径：实现版本、profile、渠道状态和凭据指纹均匹配的 active binding，或 TTL 满足且密文可解密的 source URL；
5. 对多个素材求可执行渠道/实现交集，并写入渠道分配约束；无交集或无解析路径时 fail closed；
6. 每次上游尝试前再次校验实现版本、渠道、凭据指纹、binding 状态和 source TTL，并把平台引用改写为上游托管引用或源 URL。
...
- 平台不保存素材二进制、Authorization 或 Cookie；完整源 URL 只允许保存在绑定 `asset-source:<public_id>` 作用域的最小 `AssetSource` 密文中，不增加 URL HMAC、独立业务状态或旧无作用域密文回退；明文和可重放签名不得进入其它持久化、响应、日志、metrics 或 trace；
...
平台不提供素材 content、客户可见的 Provider binding 创建或独立 asset-group 管理 API，也不提供平台真人同意页、回执页或自然人同意证据接口。
...
- 多素材路由使用全部素材在 binding/source 策略下的可执行渠道/实现交集；
...
- 未经分类的请求级媒体不会被错误记录成“已通过真人技术审核”，混合媒体路径按 SKU 明确
  能力 fail closed；
- 同一 Asset 在 binding 渠道和 `source_url` 渠道上按同一所有权/授权规则解析；已声明 TTL 不足、过期和无法解密时关闭 `source_url` 路径，`expires_at=0` 按 best-effort 处理，且不泄露明文 URL；
