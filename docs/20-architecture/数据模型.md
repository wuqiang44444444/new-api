...
| 素材控制面 | `Asset`、binding、ownership claim、authorization、operation job | 平台素材 ID、上游账号映射、唯一归属、真人同意和可恢复生命周期 |
...
| 素材路由快照 | `asset_public_ids`、`asset_binding_ids` | 审计创建时使用的平台素材与上游绑定 |
...
- `client_protocol` 区分 `openai_images`、ModelArk、Kling、即梦和历史只读协议，查询响应按创建时协议投影；
...
下图与现有表述为当前已实现的单主 binding 模型。[Link 资源虚拟素材库架构](Link资源虚拟素材库架构.md) 与 [ADR-0015](decisions/0015-Link公开SKU与实现身份版本绑定.md) 已接受目标模型：增加独立 `AssetSource`，并将 `Asset` 与 `AssetBinding` 扩展为 0..N 关系。在对应 migration、Resolver 和三数据库验证完成前，不得把目标模型表述为已上线表结构。
...
用户拥有的逻辑素材，公开 ID 为 `ast_` 前缀。保存名称、`general/real_person` 类型、媒体类型、授权关联和归一化状态，不直接保存源 URL、二进制、对象存储字段或上游 ID。`created_by_token_id` 仅用于审计，所有权归 `user_id`。ADR-0015 目标中的加密源引用必须放在独立 `AssetSource`，不能回填到 `assets` 通用字段。

### `asset_sources`（ADR-0015 已接受，待实施）

一个 Asset 最多关联一个不可变的短期执行引用。目标字段只包含唯一 `asset_id`、绑定该 Asset scope 的认证加密 URL 和 `expires_at`；不设置公开 ID、URL HMAC、密钥版本、独立状态、错误字段或清理任务。通过新创建 API 建立的 Asset 必须有 source；没有 source 只兼容仍有 active binding 的历史 Asset。明文 URL 只允许在 Resolver 内存中短暂存在，不进入 Task/attempt、普通日志或客户响应。源 URL 不能证明媒体相同，平台身份只由 `ast_*` 表达。

### `asset_bindings`

当前实现保存素材到上游账号的唯一主映射。关键事实包括 `channel_id`、凭据指纹、asset profile、model/target、上游 resource/reference、可选 group 和归一化状态。`asset_id + channel_id + credential_fingerprint` 唯一，敏感字段不进入用户 JSON。ADR-0015 实施后，同一 Asset 可按不同实现版本、channel/profile/凭据作用域具有 0..N 个 binding，但任一上游对象仍只能被一个平台 Asset 认领。
