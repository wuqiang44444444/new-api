...
本文聚焦 `DoubaoVideo` 异步视频任务链路，并明确 Seedance、Kling、即梦三类北向合同与共享任务底座的边界；不建设任意协议脚本或独立视频管理系统。
...
| Kling v1 `/kling/v1/videos/*` | Kling 原生接入 | 文生/图生路径、Kling 状态与响应外壳 |
| 即梦 `CVSync2Async*` | 即梦原生接入 | Action/Version、即梦状态与响应外壳 |
