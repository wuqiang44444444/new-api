...
> 本文记录视频官方北向合同、共享异步 Task 与南向渠道适配的当前架构。`DoubaoVideo` 是 Seedance 等字节视频渠道的执行类型，不是所有视频北向合同的代名词。
...
本文覆盖 Seedance、Kling、即梦三类官方北向合同和共享异步任务底座。`video_upstream_profile` 目前只描述 `DoubaoVideo` 渠道的南向协议；Kling、即梦由各自渠道 adapter 执行。系统不建设任意协议脚本或第二套视频任务系统。

### 1.1 当前实现状态

截至 2026-07-28，代码已实现：

- Seedance、Kling、即梦三套类型化官方北向入口和稳定 `contract_id`；
- 官方 DTO 白名单校验、合同错误投影和南向请求转换；
- 任务快照中的北向合同版本、南向适配器版本、执行连接和计费事实；
- OpenAI 视频创建下线，仅保留存量只读入口；
- ModelArk 北向与 `official`、`third_party_reverse_proxy`、`third_party_relay` 南向 profile 解耦；
- 素材渠道交集约束和每次南向尝试前的 `asset://` 引用改写。
...
北向和南向各有且只有一个显式协议选择变量：

- `contract_id` 决定客户端请求、响应、错误和生命周期投影；
- `video_upstream_profile` 决定 `DoubaoVideo` 渠道如何调用上游。

两者正交。profile 表达可测试的南向请求/响应合同，不表达北向产品、供应商、域名、模型版本或价格。系统不能从供应商名称、Base URL、模型名、Key 格式或请求头推断任一合同。
...
| 北向合同转换层 | 严格 DTO 白名单校验，生成带 `contract_id` 的内部请求 | 不选择渠道，不透传未知字段 |
| 渠道分配器 | 按模型、分组、优先级和权重选择渠道及单个 Key | 不按模型名或地址猜测 profile |
| Task Relay | 校验请求、应用模型映射、估算与预扣、调用 adaptor、持久化任务 | 不实现供应商专用协议字段 |
| 南向渠道 adapter | 按渠道类型/profile 构造 URL、鉴权与请求，并归一化任务状态 | 不决定北向响应外壳，不维护第二套 Task |
...
Seedance 使用 ModelArk v3 官方北向协议；Kling 和即梦使用各自官网协议。`video_upstream_profile` 是 new-api 调用上游的南向协议，二者必须解耦：
...
北向长期合同为三套厂商官方薄适配层，而不是三套任务系统：

| 北向协议 | `contract_id` / 版本 | 创建与查询入口 | 独立合同 |
| --- | --- | --- | --- |
...
| --- | --- | --- | --- |
| 北向合同 | Router 与类型化合同转换 | `Task.client_protocol`、`private_data.northbound_contract_id/version` | 决定后续查询、错误和响应投影 |
| 南向 adapter 版本 | 选中渠道类型与 profile | `Task.private_data.southbound_adapter_version` | 标识创建任务时使用的转换实现 |
| DoubaoVideo 南向 profile | `ChannelOtherSettings.video_upstream_profile` | `Task.private_data.video_upstream_profile` | 决定该渠道的创建与查询协议 |
...
所有正式北向视频任务共享持久化 `AsyncBilling` 门闩。普通按次/倍率任务使用冻结的 `PerCallBilling` 语义；表达式任务额外冻结表达式、计费探针和预扣上界。成功、失败、取消、过期后的结算、退款和补偿统一遵循 [ADR-0008](decisions/0008-共享异步任务计费状态机与原子补偿.md)。
...
1. `contract_id` 是北向投影唯一合同键，profile/渠道 adapter 是南向执行唯一合同键；二者不得互相推断或覆盖。
...
- 三套官方入口分别冻结稳定 `contract_id`/版本和南向 adapter 版本，任务查询按原 `client_protocol` 投影；
- 未知北向字段在合同转换层拒绝，不能落入 metadata 或任意 map 后透传；
...
- 历史重构现场：[视频官方北向与统一素材库合同重构方案](../99-archive/2026-07-28-视频官方北向与统一素材库合同重构方案.md)
