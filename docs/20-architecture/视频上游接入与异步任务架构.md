...
> 本文记录视频 Link 合同、共享异步 Task 与渠道适配的当前架构。`DoubaoVideo` 是 Seedance 等字节视频渠道的执行类型，不是所有视频 Link 合同的代名词。
...
本文覆盖 rc23 原生 OpenAI Videos、旧版平台视频，以及 Seedance、Kling、即梦三类 Link 合同中的官方协议和共享异步任务底座。`video_upstream_profile` 目前只描述 `DoubaoVideo` 渠道的渠道适配协议；Sora、Kling、即梦由各自渠道 adapter 执行。系统不建设任意协议脚本或第二套视频任务系统。

### 1.1 当前实现状态

截至 2026-08-01，代码已实现：

- Seedance、Kling、即梦三套类型化官方客户 API 合同入口和稳定 `contract_id`；
- 官方 DTO 白名单校验、合同错误投影和 Provider 请求转换；
- 任务快照中的 Link 合同版本、渠道适配协议版本、执行连接和计费事实；
- rc23 原生 OpenAI Videos 创建、Remix、查询、内容与旧版平台视频创建/查询入口；
- 原生视频与显式 Link SKU 隔离，原生入口拒绝 Link SKU，原生任务不冻结 Link implementation；
- ModelArk 客户端与 `official`、`third_party_reverse_proxy`、`third_party_relay`、
  `third_party_json_video_omni_reference` 渠道 profile 解耦；
- `southbound_adapter_version` 已从审计字符串升级为实际执行键；创建、轮询和内容代理按任务冻结的
  渠道适配协议版本分发，不用当前代码版本覆盖在途任务合同；
- 公开 SKU 以 `VideoSKUCapability` 为运行时唯一权威；五个 Seedance 2.0 固定分辨率 SKU
  同时投影为 OpenAPI `ModelArkVideoCreateRequest.x-fixed-seedance-2-capability`
  机器合同，一致性测试逐值比较模型、时长、画幅、媒体上限、禁用字段和生命周期；
- JSON Video omni-reference 创建只有一条可达路径：类型化 ModelArk 请求 + 冻结
  capability + 映射后上游模型进入 `jsonvideo.CreateRequest`；通用 body 重解析和备用
  builder 已禁止；
- 平台托管素材的渠道交集约束和每次 Provider 尝试前的 `asset://` 引用改写；官方合同允许的请求级
  URL/Data URL 不进入 Asset/Binding 生命周期。

仍需通过生产配置和灰度持续证明全部启用渠道的合同等价性，并完成 OpenAI/Sora 真实 Provider、价格和错误语义验收。代码存在不等于某个模型或渠道已对所有分组发布。

已接受但尚需按新增上游实施的结构化边界包括：版本化 `VideoSKUCapability`、Ability 发布时的
候选能力等价校验、每次创建的发送前 durable attempt，以及轮询合同违例的
`reconciliation_required`。在这些边界实现和故障注入完成前，不得用人工配置约定替代。

## 2. 架构原则

### 2.1 协议显式，供应商中立

Link 合同中的公开语义与渠道适配协议各有且只有一个显式键：

- `contract_id` 决定客户端请求、响应、错误和生命周期投影；
- `video_upstream_profile + southbound_adapter_version` 组成渠道适配协议键：profile 决定
  `DoubaoVideo` 的协议族，冻结版本决定同一协议族内不兼容的结果与凭据语义。

两者正交。渠道 adapter 键表达可测试的请求/响应合同，不表达 Link 合同、供应商、域名、模型版本
或价格。系统不能从供应商名称、Base URL、模型名、Key 格式或请求头推断任一合同。
...
公开 SKU 的版本化能力声明决定客户端可提交什么以及可依赖哪些生命周期动作；渠道/profile
只回答自己能否完整实现。不能从当前候选渠道动态计算 SKU 能力合同，也不能选中渠道后再删字段或
改变 DELETE 语义。规范化、配置发布校验、运行时过滤和发送前复检统一遵循
[ADR-0010](decisions/0010-视频公开SKU能力与候选渠道等价.md)。
...
flowchart LR
    ...
    Client --> Router --> Contract --> Distributor --> Relay --> Adaptor
    Channel --> Distributor
    Channel --> Adaptor
    Pricing --> Relay
    Adaptor --> Official
...
| Link 合同转换层 | 严格 DTO 白名单校验，生成带 `contract_id` 的内部请求 | 不选择渠道，不透传未知字段 |
...
| 任务轮询服务 | 读取任务快照、查询上游、归一化状态、推进终态 | 不重新选择渠道适配协议或账号 |
...
| `third_party_json_video_omni_reference` | 转换为 JSON video omni/reference 请求 | 使用渠道 Base URL、创建路径和查询模板 | `processing`/`in_progress` 均归一为运行中；只执行 adapter v2 结果合同 |
...
`third_party_json_video_omni_reference` 只支持
`54:third_party_json_video_omni_reference:v2`：

- `completed` 顶层必须提供非空绝对 HTTPS `video_url`，且无 userinfo、长度受控并与冻结
  Base URL 的 scheme、host、port 同源；通过校验后保存到 `private_data.result_url`，回源不发送
  Authorization；
- 缺失、畸形或跨域结果 URL 时进入 `reconciliation_required`，不得回退固定路径；
- `queued`、`in_progress`、`processing`、`completed`、`failed` 之外的状态继续 fail closed。

这里的 v2 是网关渠道执行合同版本，不是 Provider 模型版本或客户 API 合同版本。开发期曾按文档
推定定义固定内容端点 + Bearer 的 omni v1，但真实 Provider 实测证明该合同不成立；由于没有
需要保留的生产历史任务，v1 执行分支已删除。客户端不选择 adapter version。

未经真实账单验证的第三方 usage 不得伪装为官方 Ark usage。协议兼容不等于计费语义兼容。

### 5.4 Link 客户协议与渠道适配协议

Seedance 使用 ModelArk v3 官方客户端协议；Kling 和即梦使用各自官网协议。`video_upstream_profile` 是 new-api 调用上游的渠道适配协议键，二者必须解耦：

```text
ModelArk v3 请求
  -> 内部统一视频任务
  -> official / third_party_reverse_proxy / third_party_relay / third_party_json_video_omni_reference
```
...
- 未知 profile 必须拒绝，不能借客户端转换绕过渠道适配协议实现；
...
Link 合同层先捕获本次请求不可变的 `resolved_sku_capability`（版本与内容 hash），再用该快照
完成字段默认值和等价显式值规范化。后续步骤不得重新读取“当前版本”：

1. 使用冻结能力版本读取该 SKU 的 Ability 候选；
...
这些合同共享内部 Task、渠道分配、素材约束、轮询、计费和审计主链路，但不得互用 DTO、状态或错误外壳。rc23 原生 `/v1/videos` 创建、Remix 和查询保持可用；显式登记的 Link SKU 在原生入口 fail closed，只能使用自己的类型化合同。

## 6. 配置与状态所有权

| 事实 | 创建新任务时的来源 | 在途任务的执行事实 | 说明 |
| --- | --- | --- | --- |
| Link 合同 | Router 与类型化合同转换 | `Task.client_protocol`、`private_data.northbound_contract_id/version` | 决定后续查询、错误和响应投影 |
| SKU 能力 | 公开 SKU 能力注册 | `private_data.sku_capability_version/lifecycle_capabilities` | 决定规范化、DELETE、内容和生命周期投影 |
| 渠道适配协议版本 | 选中渠道类型与 profile | `Task.private_data.southbound_adapter_version` | 标识创建任务时使用的转换实现 |
...
JSON Video omni-reference 的创建请求在 adapter 入口已持有类型化请求、已解析的
`VideoSKUCapability` 和映射后上游模型，必须直接交给唯一 `jsonvideo.CreateRequest`
builder。不得再从 JSON body 反向解析公开模型、重新选择 capability 或保留第二套请求
...
403 或其他 HTTP 状态不能证明任务未创建；当前没有登记该类飞彩合同，因此所有发送后异常 HTTP
响应均进入 `unknown`。后续确认合同时只能增加精确组合，不能把整个状态码注册为确定拒绝。
...
轮询优先使用任务快照。adapter version 必须实际参与 decoder 分发；omni 任务缺少版本时按唯一
支持的 v2 解释，显式 v1、未知、畸形或与 profile 不匹配的版本进入合同违例对账。其他历史任务缺少某项连接
快照时可以回退渠道当前值，但该路径只用于数据兼容，并应可由测试单独识别。
...
- OpenAI Videos 响应沿用 rc23 原生投影；空协议 rc23 存量任务仅在同一应用且模型可明确识别为非 Link 时兼容读取。
...
所有正式客户端视频任务共享持久化 `AsyncBilling` 门闩。普通按次/倍率任务使用冻结的 `PerCallBilling` 语义；表达式任务额外冻结表达式、计费探针和预扣上界。成功、失败、取消、过期后的结算、退款和补偿统一遵循 [ADR-0008](decisions/0008-共享异步任务计费状态机与原子补偿.md)。
...
- omni Task 缺少 adapter version 时按唯一 v2 合同解释；所有新 Task、create attempt 和 recovery
  snapshot 必须由同一权威函数写入显式 v2。开发期的显式 omni v1 已废弃并 fail closed。
- 第三方新增字段默认不进入内部合同。只有经过验证、具有稳定业务语义且不破坏既有调用方时才扩展归一化结构。
- 协议合同发生不兼容变化时新增 adapter version 或 profile，不在原 profile 下按供应商地址分叉
  行为；只有改变本架构总不变量时才新增 ADR。

## 14. 架构不变量

以下条件必须长期成立：

1. `contract_id` 是客户 API 投影唯一合同键，profile/适配协议版本是渠道执行唯一合同键；二者不得互相推断或覆盖。
...
14. 公开 SKU 能力是客户端权威；Ability 候选在发布和运行时都必须完整实现该能力，生命周期
    不得随选中 profile 漂移。
15. 每个异步创建在出站前拥有耐久 create attempt；`sending/unknown` 不自动退款或重发。
...
- 三套官方入口分别冻结稳定 `contract_id`/版本和渠道适配协议版本，任务查询按原 `client_protocol` 投影；
...
- SKU 规范化接受缺失值与等价显式值、拒绝冲突值；Ability 发布和运行时都排除生命周期或媒体
  能力不等价的候选；
- 五个固定分辨率 Seedance 2.0 SKU 的 OpenAPI 机器合同与运行时 capability
  逐值比较；任一 duration、resolution、ratio、媒体上限、禁用字段或生命周期漂移使 CI 失败；
- JSON Video omni-reference 只通过类型化 `jsonvideo.CreateRequest` 创建，通用请求转换入口
...
- [ADR-0016：原生 OpenAI Videos 与 Link 合同并存边界](decisions/0016-原生OpenAI-Videos与Link合同并存边界.md)
