...
本文覆盖 Seedance、Kling、即梦三类 Link 合同中的官方协议和共享异步任务底座。`video_upstream_profile` 目前只描述 `DoubaoVideo` 渠道的渠道适配协议；Kling、即梦由各自渠道 adapter 执行。系统不建设任意协议脚本或第二套视频任务系统。
...
仍需通过生产配置和灰度持续证明全部启用渠道的合同等价性，并完成 OpenAI 历史读取、旧入口和兼容别名的退出。代码存在不等于某个模型或渠道已对所有分组发布。
...
三者共享内部 Task、渠道分配、素材约束、轮询、计费和审计主链路，但不得互用 DTO、状态或错误外壳。OpenAI 视频模型已下架，`/v1/videos` 不再创建或 Remix，仅在迁移期保留存量任务读取。
...
- 存量 OpenAI 视频只读兼容响应沿用历史投影，不参与本规则，也不得用于创建新任务。
