...
> 本文定义基于现有 `DoubaoVideo` 通道接入官方与第三方视频上游的目标架构合同。代码、运维文档和端到端验收全部对齐后，本文状态才可由 `draft` 更新为 `current`。
...
普通异步任务继续使用既有按次/倍率计费。表达式计费任务的预扣、冻结快照、终态结算、退款和补偿由独立计费状态机承担，详见 [ADR-0002](decisions/0002-异步任务表达式计费快照与补偿结算.md)。在该 ADR 转为 accepted 前，不能用协议链路通过来替代计费正确性验收。
