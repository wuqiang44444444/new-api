...
| `third_party_json_video_media_arrays` | 转换为 `duration/size/images/audios` JSON 请求 | 使用 HTTPS Base URL、`/v1/videos` 与查询模板 | 只接受 `queued/processing/completed/failed`；adapter v1 以 `id` 为唯一任务身份 |
...
这里的 v1 是新 media-arrays 网关渠道执行合同版本，不是 Provider 模型版本或客户 API 合同版本。
