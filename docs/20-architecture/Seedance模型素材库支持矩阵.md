...
| 已配置无素材组协议的模型 | `true` | 不支持 | 可直接创建普通图片、视频或音频素材；仍须逐项读取 `operations` |
| 已配置可选素材组协议的模型 | `true` | 普通素材可选；真人素材按 `media` 要求 | 逐项读取 `operations` 和 `media` |
| 已配置必需素材组协议的模型 | `true` | 普通素材必填 | 先创建组，再创建素材，并传入组 ID |
...
| `media` | 按 `kind` 与 `media_type` 描述素材组要求 | `required`、`optional`、`unsupported` 均须按返回值处理 |
...
| `/v1/assets`、`/v1/asset-groups` 单资源代理 | Link 新增能力 | 平台不建立 Asset/AssetGroup、列表、resolver 或所有权事实 |
