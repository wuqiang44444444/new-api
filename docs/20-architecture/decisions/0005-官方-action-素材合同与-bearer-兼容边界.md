...
- `official_action_assets` 使用独立于 `Channel.Key` 的 AK/SK 配置并在南向请求时签名；`Channel.Key` 始终保留为视频 Bearer API Key。独立凭据存入一对一表 `channel_asset_credentials`，普通管理响应只返回配置状态与 Access Key ID 脱敏提示。
