...
位置；本站存储位置切换不自动迁移旧对象，也不为旧记录推断缺失的位置字段。一个视频请求在首次
托管引用检查时固定存储会话，HEAD 与签发使用同一个实例。

托管创建验证实际图片格式、声明 MIME 和解码完整性；拒绝空体、损坏图片以及超过
...
之后删除只拒绝新的接受，不回查已接受素材的当前状态。正常成功 Task 与 unknown 恢复模板共用同一
素材快照写入函数；attempt 完成后清理恢复模板，长期事实保留在 Task 私有字段。
