...
本计划负责把[公开 API 文档交付架构](../20-architecture/公开API文档交付架构.md)落地到当前 React Web。架构文档定义长期边界，本计划定义当前批次、文件、门禁和完成顺序。
...
修正 Qwen `input.messages` Provider 结构、图片编辑 Content-Type、重复 operationId、显式零值、`n` 上限、同步/异步联合响应和图片 Task 错误信封。
