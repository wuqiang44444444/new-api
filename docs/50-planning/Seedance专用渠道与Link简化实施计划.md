...
`ChannelTypeSeedanceLink`，先完成新链路，再停止旧 Link 合同机制写入，最后分期清理遗留结构。
...
5. Link 主链收敛为：
   ...
   ```text
   customer model -> Group / Ability / price -> unique Channel
...
- 不根据模型名、价格、Channel、Ability 或请求参数自动推断 Link/Seedance 身份。
...
当前代码中的 ModelArk V3 创建入口仍依次经过 publication、SKU capability、实现绑定、素材路由约束
和通用 `Distribute()`；后台仍存在 Link Access Plan、implementation、publication 改绑等配置；素材侧仍
有 0..N `AssetBinding`、迁移作业和独立真人授权域。这些都是本计划要替换的旧实现，不是目标架构。

更重要的是，当前 `docs/00-context/硬约束.md` 和仓库 `AGENTS.md` 仍把旧 publication/SKU/
implementation/binding 体系写成编码硬约束。它们与 ADR-0016 已接受决策冲突。**在修改业务代码前，
必须先更新这两处硬约束及受影响的当前事实文档；在冲突解除前不得开始编码。**

该更新不是放宽 fail-closed 原则，而是把 fail-closed 边界改为新的真实合同：
...
P4 重建 Asset / AssetGroup 确定性代理
  ↓
P5 迁移图片及其他 Link 使用方，停止旧 Link 写入
  ↓
P6 删除旧门禁、接口与后台配置
  ↓
...
各阶段只能在前一阶段验收通过后进入。新 Channel 在 P7 前保持禁用；不通过运行时双写、双路由或
fallback 做灰度。
...
- 建立一次性工程盘点，列出：
  ...
  - 已有 `DoubaoVideo` 渠道、Ability、客户模型、活动 Task、AssetBinding 和真人授权数据；
...
- ModelArk V3 创建入口只接受统一官方请求结构，执行鉴权、Group/Ability、价格、租户、请求字段和资产
  作用域校验。
- 以客户模型已保存的 Ability 关系取得唯一 Seedance Channel，不调用通用 Priority/Weight 分配，不创建
  候选列表，不发生随机选择、Affinity、重选或 fallback。
...
- 为旧 Task 保留窄的只读快照解码路径，使历史任务继续查询和结算；旧 Task 不进入新 Seedance 路由。
...
- 平台不保存媒体二进制。创建成功取得可信 Provider ID 后清理 source URL；创建失败也清理 source URL，
  仅保留安全错误摘要和技术日志。
...
- 停止创建和修改 `LinkModelPublication`、Link SKU capability、`LinkImplementation`、execution binding、
  Link Access Plan、AssetBinding 和独立真人授权记录。
- 历史记录暂不物理删除；将其限定为历史 Task/资源的只读兼容数据。
...
- 删除或退役旧 Link publication、SKU capability、implementation、execution binding、candidate
...
- `rg` 和依赖分析仅在明确标注的历史只读兼容层中发现旧类型引用。
...
- 数据库采用“先加字段/表、停止旧写入、保留旧读、最后延迟删除”的顺序；首个切换版本不得同时删除
  历史表或 Task 所需字段。
- 新增迁移必须同时支持 SQLite、MySQL 和 PostgreSQL，并用 GORM/项目公共锁与列名约定实现。
- 历史 Task：保留旧冻结事实，只读兼容，不重新解释为新协议。
- 已有 `DoubaoVideo` 渠道：不原地改型。为每条 Seedance 线路新建独立 Seedance Channel 和独立客户
  模型，完成验证后再由管理员禁用旧入口。
- 历史素材：
  - 只有唯一 Channel、唯一账号、唯一 Region/Project、唯一可信 Provider 资源的记录可自动迁移；
  - 多 binding、作用域不明、Provider ID 不可信或旧真人授权数据不得猜测转换；
  - 不能无损迁移的记录保持禁用/只读，并由技术人员提供导出或重新创建说明。
- 一次性迁移报告只服务发布决策，不演化成常驻核查、修复或对账产品。

#### 真实验证

- 按 `Seedance多Provider真实验证矩阵.md` 至少完成：
...
- 回滚以“禁用新 Channel/Ability”实现，不修改已创建 Task 的冻结事实。
- 不自动恢复同名旧模型，不把失败请求转发到 `DoubaoVideo` 或其他 Seedance Provider。
- 已创建 Task 继续通过其冻结 adapter 查询、删除和结算；若代码版本回退，必须先确认回退版本仍能读取
  新快照，或暂停回退并先提供兼容读取补丁。
...
- 在确认没有活动 Task、资产或账单依赖后，另开清理版本删除旧表、旧列、历史只读 decoder 和迁移脚本。
- 物理删除前分别验证三种数据库；不得在主切换版本中顺带执行高风险 DROP。
...
- 旧 Link 运行时代码和无用数据库结构已清理，历史审计事实仍可读。
...
| `service/`、`model/` | durable attempt、Task 快照、资产一对一和迁移 | 三数据库兼容，主库为权威 |
...
- SQLite、MySQL、PostgreSQL 的增量迁移、回滚前置检查和历史数据读取。
- 旧 Task 只读快照与新 Task 快照并存。
- 可无损素材迁移、歧义素材拒绝迁移、历史记录只读。
- 无新写入旧 Link 表；无活动记录依赖时才允许物理清理。
...
7. **数据与发布**：三数据库迁移、历史只读兼容、真实 Provider 启用。
8. **延迟清理**：确认无依赖后删除旧表/列和兼容 decoder。
...
- 图片及其他 Link 使用方已迁移，旧 Link 合同没有新写入；遗留数据有明确只读或清理结论。
