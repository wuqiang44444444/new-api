...
func TestModelArkVideoCreateConvertRejectsInvalidKnownFields(t *testing.T) {
	tests := []struct {
		...
		{name: "execution expiry", body: `{"model":"m","content":[{"type":"text","text":"video"}],"execution_expires_after":3599}`},
		{name: "seed", body: `{"model":"m","content":[{"type":"text","text":"video"}],"seed":-2}`},
		{name: "frames not published", body: `{"model":"m","content":[{"type":"text","text":"video"}],"frames":121}`},
	}
...
func TestModelArkVideoChannelConstraintFiltersRelayCapabilityMismatch(t *testing.T) {
	previousDB := model.DB
	previousType := common.MainDatabaseType()
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	require.NoError(t, err)
	...
	channels := []model.Channel{
		{Name: "official", Type: constant.ChannelTypeDoubaoVideo, Status: common.ChannelStatusEnabled, Key: "official", OtherSettings: `{"video_upstream_profile":"official"}`},
		{Name: "relay", Type: constant.ChannelTypeDoubaoVideo, Status: common.ChannelStatusEnabled, Key: "relay", OtherSettings: `{"video_upstream_profile":"third_party_relay"}`},
	}
	require.NoError(t, db.Create(&channels).Error)
