...
func KlingRequestConvert() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if modelName == "" {
			abortKlingVideo(c, http.StatusBadRequest, "model_name is required")
			return
		}
		if strings.TrimSpace(videoStringValue(official.Prompt)) == "" {
			abortKlingVideo(c, http.StatusBadRequest, "prompt is required")
			return
		}
		...
		if mode := videoStringValue(official.Mode); mode != "" && mode != "std" && mode != "pro" {
			abortKlingVideo(c, http.StatusBadRequest, "mode is not supported")
			return
		}
		if duration := videoStringValue(official.Duration); duration != "" && duration != "5" && duration != "10" {
			abortKlingVideo(c, http.StatusBadRequest, "duration is not supported")
			return
		}
		if ratio := videoStringValue(official.AspectRatio); ratio != "" && ratio != "16:9" && ratio != "9:16" && ratio != "1:1" {
			abortKlingVideo(c, http.StatusBadRequest, "aspect_ratio is not supported")
			return
		}
		if official.CfgScale != nil && (*official.CfgScale < 0 || *official.CfgScale > 1) {
			abortKlingVideo(c, http.StatusBadRequest, "cfg_scale must be between 0 and 1")
			return
		}
