...
func TestBatchContractTokenAuthKeepsNativeGroupValidation(t *testing.T) {
	...
	// A group the user can no longer use is rejected whether or not the key
	// binds a contract: contracts provide discounts, never group exemptions.
	...
	for _, tc := range []struct {
		...
		assert.Equal(t, 403, staleRecorder.Code, staleRecorder.Body.String())
	}
}
