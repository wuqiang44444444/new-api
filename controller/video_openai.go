...
func OpenAIVideoList(c *gin.Context) {
	...
	tasks, hasMore, err := model.ListOpenAIVideoTasks(c.GetInt("id"), strings.TrimSpace(c.Query("after")), limit, order)
	if err != nil {
...
func OpenAIVideoGet(c *gin.Context) {
	task, exists, err := model.GetVideoTaskForProtocol(c.GetInt("id"), c.Param("task_id"), model.TaskClientProtocolOpenAIVideos, false)
	if err != nil {
...
func OpenAIVideoDelete(c *gin.Context) {
	...
	deleted, err := model.MarkVideoTaskClientDeleted(c.GetInt("id"), task.TaskID, model.TaskClientProtocolOpenAIVideos)
	if err != nil || !deleted {
