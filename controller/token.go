...
func AddToken(c *gin.Context) {
	...
	if token.ContractId != 0 {
		if _, err := model.GetContractEntityOwnedByUser(token.ContractId, c.GetInt("id")); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "contract does not exist or is not owned by the user"})
			return
		}
	}
	...
	common.SetContextKey(c, constant.ContextKeyTokenAuditSucceeded, true)
	service.NotifyTokenCreated(c.GetInt("id"))
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "",
	})
...
func UpdateToken(c *gin.Context) {
	...
	if token.ContractId != 0 {
		if _, err := model.GetContractEntityOwnedByUser(token.ContractId, userId); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "contract does not exist or is not owned by the user"})
			return
		}
	}
	...
	if statusOnly != "" {
		...
		cleanToken.ContractId = token.ContractId
		if token.Group != "auto" {
