...
// 分页大小为 1 的列表查询；视频协议没有无副作用视频探针时，自动任务将探针不可用表达为不支持，
// 绝不退回 Chat 测试。
func seedanceLinkChannelHealthResult(ctx context.Context, channel *model.Channel) (result testResult) {
	if ctx == nil {
	...
	if err := service.CheckAssetChannelConnectivity(ctx, channel); err != nil {
		...
		return testResult{localErr: fmt.Errorf("seedance link asset probe failed: %w", err), checkUnsupported: service.ChannelConnectivityErrorCode(err) == service.ChannelConnectivityAssetUnsupported}
	}
	return testResult{}
}
