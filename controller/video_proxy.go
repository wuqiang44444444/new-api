...
func VideoProxy(c *gin.Context) {
	...
	if err := service.AuthorizeTaskContent(task); err != nil {
		if errors.Is(err, service.ErrTaskContentAuthorizationRevoked) {
			videoProxyTaskError(c, task, http.StatusForbidden, "permission_error", "real_person_authorization_revoked", "Content access authorization has been revoked")
			return
		}
		logger.LogError(c.Request.Context(), fmt.Sprintf("Failed to authorize task content %s: %s", taskID, err.Error()))
		videoProxyTaskError(c, task, http.StatusInternalServerError, "server_error", "internal_error", "Failed to authorize video content")
		return
	}
	...
	case constant.ChannelTypeDoubaoVideo:
		contentURL, key, handled, sourceErr := jsonVideoContentSource(task)
		if sourceErr != nil {
			videoProxyTaskError(c, task, http.StatusBadGateway, "server_error", "frozen_upstream_unavailable", "Frozen video connection details are unavailable")
			return
		}
