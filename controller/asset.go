...
func ListAssets(c *gin.Context) {
	...
	assets, total, err := model.ListAssetsByApp(c.GetInt("id"), c.GetInt("token_id"), (page-1)*pageSize, pageSize, model.AssetListFilter{Status: strings.TrimSpace(c.Query("status")), AssetKind: strings.TrimSpace(c.Query("asset_kind")), MediaType: strings.TrimSpace(c.Query("media_type")), Name: strings.TrimSpace(c.Query("name"))})
	if err != nil {
...
func UpdateAsset(c *gin.Context) {
	...
	asset, err := service.RenameAssetForApp(c.GetInt("id"), c.GetInt("token_id"), c.Param("asset_id"), req.Name)
	if err != nil {
...
func DeleteAsset(c *gin.Context) {
	...
	if err := service.DeleteAssetForApp(c.Request.Context(), c.GetInt("id"), c.GetInt("token_id"), asset.PublicID); err != nil {
