...
// Gemini/Vertex video URL extraction helpers, re-homed from the
// upstream-removed controller/video_proxy_gemini.go (rc.31 replaced the
// native video proxy with the task plugin pipeline). They persist for the
// Link video proxy surfaces in controller/video_proxy_link.go.
