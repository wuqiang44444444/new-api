...
func UpdateOption(c *gin.Context) {
	...
	case "batch_billing_setting.batch_billing_expr":
		if err := service.ValidateBatchBillingConfiguration(option.Value.(string)); err != nil {
			common.ApiErrorMsg(c, err.Error())
			return
		}
	case "billing_setting.billing_expr":
		...
		for _, modelName := range models {
			...
			if handled, linkErr := model.ValidateSeedanceBillingExpression(modelName, expression); handled {
				err = linkErr
