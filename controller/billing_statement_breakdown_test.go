...
func TestGetUserBillingStatementBreakdownRejectsInvalidQueryBoundaries(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			query:       "start_timestamp=1000&end_timestamp=2000&token_id=0",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			context.Request = httptest.NewRequest(
				http.MethodGet,
				"/api/billing/self/breakdown?"+test.query,
				nil,
			)

			GetUserBillingStatementBreakdown(context)
