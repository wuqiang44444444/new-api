...
func TestShouldRetryTaskRelayHonorsPinRetryMode(t *testing.T) {
	...
	c := newPinRetryContext()
	assert.True(t, shouldRetryTaskRelay(c, nil, taskErr, 1))
	...
	assert.True(t, shouldRetryTaskRelay(origin, nil, taskErr, 1))
	...
	assert.False(t, shouldRetryTaskRelay(token, nil, taskErr, 1))
}
