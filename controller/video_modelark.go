...
func ModelArkVideoList(c *gin.Context) {
	...
	tasks, total, err := model.ListModelArkVideoTasks(c.GetInt("id"), c.GetInt("token_id"), model.ModelArkTaskListFilter{
		Statuses:    statuses,
		TaskIDs:     taskIDs,
...
func ModelArkVideoGet(c *gin.Context) {
	task, exists, err := model.GetVideoTaskForProtocol(c.GetInt("id"), c.GetInt("token_id"), c.Param("task_id"), model.TaskClientProtocolModelArkV3, false)
	if err != nil {
...
func ModelArkVideoDelete(c *gin.Context) {
	...
	lifecycle := relay.TaskLifecycleCapabilities(task)
	switch task.Status {
	case model.TaskStatusNotStart, model.TaskStatusSubmitted, model.TaskStatusQueued, model.TaskStatusUnknown:
		if !lifecycle.SupportsCancelQueued {
			modelArkVideoError(c, http.StatusConflict, "cancellation_unsupported", "the selected video model does not support cancellation")
			return
		}
		modelArkCancelQueuedTask(c, task)
	case model.TaskStatusInProgress, model.TaskStatusReconciliationRequired:
	...
	case model.TaskStatusSuccess, model.TaskStatusFailure, model.TaskStatusProviderContractFailure, model.TaskStatusExpired:
		if !lifecycle.SupportsDeleteTerminal {
			modelArkVideoError(c, http.StatusConflict, "delete_unsupported", "the selected video model does not support deletion")
			return
		}
...
func modelArkCancelQueuedTask(c *gin.Context, current *model.Task) {
	begin, err := model.BeginTaskCancellation(c.GetInt("id"), c.GetInt("token_id"), current.TaskID, model.TaskClientProtocolModelArkV3)
...
func modelArkDeleteTerminalTask(c *gin.Context, task *model.Task) {
	...
	deleted, err := model.MarkVideoTaskClientDeleted(c.GetInt("id"), c.GetInt("token_id"), task.TaskID, model.TaskClientProtocolModelArkV3)
	if err != nil || !deleted {
