...
// Seedance and MiniMax Link tasks are the registered exceptions: their USD expression is
...
func rejectTaskUsageAsyncBilling(ctx context.Context, task *model.Task) bool {
	if task.PrivateData.AsyncBilling == nil || !task.HasTaskUsageBilling() || task.HasTypedVideoBillingFacts() {
		return false
	}
