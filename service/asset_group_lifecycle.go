...
func prepareAutomaticAssetGroupCreate(asset *model.Asset, candidate *model.AssetGroupBinding) (*model.AssetGroupBinding, bool, error) {
	...
	err := model.DB.Transaction(func(tx *gorm.DB) error {
		if candidate.AuthorizationID != nil {
			...
			if authorization.UserID != asset.UserID || authorization.AppID != asset.AppID || authorization.EndUserSubjectHash == "" || authorization.EndUserSubjectHash != asset.EndUserSubjectHash || authorization.Status != model.RealPersonAuthorizationAuthorized || authorization.RevokedAt != 0 {
