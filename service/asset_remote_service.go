...
func createRemoteAsset(ctx context.Context, userID, tokenID int, userGroup, usingGroup, idempotencyKey, endpoint string, idempotencyRequest any, req dto.CreateAssetRequest, migration *assetMigrationMetadata) (*model.Asset, error) {
	...
	if rawIdempotencyKey != "" {
		...
		idempotency, err = remoteAssetIdempotency(userID, rawIdempotencyKey, endpoint, idempotencyRequest)
		if err != nil {
			return nil, err
		}
	...
	if req.AssetKind == model.AssetKindRealPerson {
		if req.MediaType != "image" || req.Model == "" {
			return nil, fmt.Errorf("%w: real-person remote assets must be model-bound images", ErrAssetBindingInvalidRequest)
		}
	...
	asset := &model.Asset{
		UserID: userID, CreatedByTokenID: tokenID, Name: req.Name,
	...
	if req.AssetKind == model.AssetKindRealPerson {
		authorization, err := model.GetRealPersonAuthorization(userID, req.AuthorizationID)
		if err != nil {
			return nil, err
		}
	...
	asset, binding, replay, err := model.CreateRemoteAssetWithQuota(asset, binding, idempotency, config.MaxAssetsPerUser, config.CreateUnknownTTLSeconds)
	if errors.Is(err, model.ErrAssetIdempotencyConflict) {
		return nil, ErrIdempotencyConflict
	}
...
func remoteAssetIdempotency(userID int, rawKey, endpoint string, request any) (*model.AssetCreateIdempotency, error) {
	if rawKey == "" {
		return nil, nil
	}
	if len(rawKey) > 191 {
	...
	return &model.AssetCreateIdempotency{
		...
		KeyHash:     common.GenerateHMAC(fmt.Sprintf("asset-create-key\n%d\n%s\n%s", userID, endpoint, rawKey)),
