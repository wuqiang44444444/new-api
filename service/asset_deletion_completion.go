...
func markAssetBindingDeletionFailed(binding *model.AssetBinding, asset *model.Asset, code, message string) error {
	...
	return model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(currentAsset).
			Where("status IN ?", []string{model.AssetStatusDeleting, model.AssetStatusDeletionFailed}).
			Updates(map[string]any{"status": model.AssetStatusDeletionFailed, "error_code": code, "error_message": message, "updated_at": now}).Error; err != nil {
			return err
		}
		return markRealPersonCleanupStatusTx(tx, currentAsset.AuthorizationID, model.RealPersonCleanupPartial, code)
	})
}

func markRealPersonCleanupStatusTx(tx *gorm.DB, authorizationID *int64, status, errorCode string) error {
	if authorizationID == nil {
		return nil
	}
	return tx.Model(&model.RealPersonAuthorization{}).
		Where("id = ? AND status = ?", *authorizationID, model.RealPersonAuthorizationRevoked).
		Updates(map[string]any{"cleanup_status": status, "error_code": errorCode, "updated_at": common.GetTimestamp()}).Error
}
...
func finalizeRealPersonAuthorizationIfCleanTx(tx *gorm.DB, authorizationID *int64) error {
	...
	if remainingAssets == 0 && remainingGroups == 0 {
		return tx.Model(&model.RealPersonAuthorization{}).Where("id = ? AND status = ?", *authorizationID, model.RealPersonAuthorizationRevoked).Updates(map[string]any{"status": model.RealPersonAuthorizationDeleted, "cleanup_status": model.RealPersonCleanupComplete, "error_code": "", "updated_at": common.GetTimestamp()}).Error
	}
	return nil
}
