...
func TestOpenRealPersonVerificationIsReusableAndPrefetchSafe(t *testing.T) {
	truncate(t)
	authorization := model.RealPersonAuthorization{
		UserID: 99, AppID: 501, EndUserSubjectHash: "subject-hash",
		Status: model.RealPersonAuthorizationVerifying,
	}
	require.NoError(t, model.DB.Create(&authorization).Error)
	session := model.RealPersonVerificationSession{
		AuthorizationID: authorization.ID, Status: "verifying", ExpiresAt: common.GetTimestamp() + 600,
	}
	require.NoError(t, model.DB.Create(&session).Error)
	rawToken := "verification-prefetch-safe-token"
	tokenHash := hashAssetToken(rawToken)
	ciphertext, err := common.EncryptShortLivedSecretForScope(
		realPersonVerificationSecretScope(authorization.ID, session.ID),
		"https://verify.example/session",
	)
	require.NoError(t, err)
	require.NoError(t, model.DB.Model(&session).Updates(map[string]any{
		"verification_token_hash": tokenHash,
		"h5_url_ciphertext":       ciphertext,
	}).Error)

	first, err := OpenRealPersonVerification(context.Background(), rawToken)
	require.NoError(t, err)
	second, err := OpenRealPersonVerification(context.Background(), rawToken)
	require.NoError(t, err)
	assert.Equal(t, "https://verify.example/session", first)
	assert.Equal(t, first, second)

	var persisted model.RealPersonVerificationSession
	require.NoError(t, model.DB.First(&persisted, session.ID).Error)
	require.NotNil(t, persisted.VerificationTokenHash)
	assert.Equal(t, tokenHash, *persisted.VerificationTokenHash)
	assert.Equal(t, ciphertext, persisted.H5URLCiphertext)
}
...
func TestVerificationSessionUnknownCreateWaitsForTTLBeforeRetry(t *testing.T) {
	authorization := seedArkRetryAuthorization(t, 205)
	originalHTTPClient := httpClient
	var createCalls atomic.Int32
	httpClient = &http.Client{Transport: assetVerificationRoundTripper(func(*http.Request) (*http.Response, error) {
		createCalls.Add(1)
...
func TestVerificationSessionDefinitiveCreateFailuresCloseClaim(t *testing.T) {
	tests := []struct {
		...
		{name: "upstream rejection", statusCode: http.StatusBadRequest, body: `{}`},
		{name: "successful response without session id", statusCode: http.StatusOK, body: `{}`},
	}
	for index, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			authorization := seedArkRetryAuthorization(t, 210+index)
			originalHTTPClient := httpClient
			httpClient = &http.Client{Transport: assetVerificationRoundTripper(func(*http.Request) (*http.Response, error) {
				return &http.Response{
...
func TestConcurrentVerificationRetryCreatesOnlyOneUpstreamSession(t *testing.T) {
	authorization := seedArkRetryAuthorization(t, 206)
...
func TestVerificationCreateResultAfterRevokeIsRecordedAsOrphaned(t *testing.T) {
	authorization := seedArkRetryAuthorization(t, 207)
	entered, release, createCalls := useBlockingVerificationCreate(t)
...
func TestArkGroupCleanupStopsAtManualRequiredWithoutGuessingEndpoint(t *testing.T) {
	truncate(t)
	baseURL := "https://moxing.example"
	channel := model.Channel{
		Type: constant.ChannelTypeDoubaoVideo, Key: "moxing-key", Status: common.ChannelStatusEnabled,
		Name: "moxing-assets", BaseURL: &baseURL, Group: "default",
		OtherSettings: `{"asset_upstream_profile":"ark_assets","video_upstream_profile":"third_party_reverse_proxy"}`,
	}
	require.NoError(t, model.DB.Create(&channel).Error)
	authorization := model.RealPersonAuthorization{
		UserID: 303, AppID: 701, EndUserSubjectHash: "subject-hash",
		Status: model.RealPersonAuthorizationRevoked, CleanupStatus: model.RealPersonCleanupPending,
	}
	require.NoError(t, model.DB.Create(&authorization).Error)
	group := model.AssetGroupBinding{
		UserID: 303, AuthorizationID: &authorization.ID, ScopeKey: "authorization:manual-cleanup",
		ChannelID:             channel.Id,
		CredentialFingerprint: model.AssetCredentialFingerprint(baseURL, "moxing-key", string(dto.AssetUpstreamProfileArk)),
		UpstreamProfile:       string(dto.AssetUpstreamProfileArk), GroupKind: "real_person",
		UpstreamResourceID: "group-upstream-1", Status: model.AssetBindingStatusDeleting,
	}
	require.NoError(t, model.DB.Create(&group).Error)
	job := model.AssetOperationJob{
		IdempotencyKey: "delete-group-manual-required", Kind: "delete_group", GroupBindingID: &group.ID,
		Status: model.AssetJobRunning, LockedBy: "runner", LockedUntil: common.GetTimestamp() + 60,
	}
	require.NoError(t, model.DB.Create(&job).Error)

	require.NoError(t, processAssetGroupDeletion(context.Background(), &job))
	require.NoError(t, model.DB.First(&group, group.ID).Error)
	require.NoError(t, model.DB.First(&authorization, authorization.ID).Error)
	require.NoError(t, model.DB.First(&job, job.ID).Error)
	assert.Equal(t, model.AssetBindingStatusDeletionFailed, group.Status)
	assert.Equal(t, "upstream_group_delete_manual_required", group.ErrorCode)
	assert.Equal(t, model.RealPersonCleanupManualRequired, authorization.CleanupStatus)
	assert.Equal(t, model.AssetJobSucceeded, job.Status)
}
...
func seedArkRetryAuthorization(t *testing.T, userID int) *model.RealPersonAuthorization {
	t.Helper()
	truncate(t)
	baseURL := "https://ark.example"
	channel := model.Channel{
	...
	require.NoError(t, model.DB.Create(&channel).Error)
	authorization := &model.RealPersonAuthorization{
		UserID: userID, AppID: userID + 1000, EndUserSubjectHash: "subject-hash", RequestedModel: "video-model", ChannelID: channel.Id,
		CredentialFingerprint: model.AssetCredentialFingerprint(baseURL, "single-key", string(dto.AssetUpstreamProfileArk)),
