...
func selectAssetChannel(userID int, userGroup, usingGroup string, asset *model.Asset, modelName, target string) (*model.Channel, dto.AssetUpstreamProfile, error) {
	...
	if asset.AssetKind == model.AssetKindRealPerson {
		...
		if err := model.DB.First(&authorization, "id = ? AND user_id = ? AND app_id = ? AND end_user_subject_hash = ?", *asset.AuthorizationID, userID, asset.AppID, asset.EndUserSubjectHash).Error; err != nil {
			return nil, "", err
		}
	...
	case "delete_group":
		if job.GroupBindingID != nil {
			if err := model.DB.Transaction(func(tx *gorm.DB) error {
				...
				if err := tx.First(&group, "id = ?", *job.GroupBindingID).Error; err != nil {
					return err
				}
				...
				return markRealPersonCleanupStatusTx(tx, group.AuthorizationID, model.RealPersonCleanupPartial, "delete_exhausted")
			}); err != nil {
...
func processAssetGroupDeletion(ctx context.Context, job *model.AssetOperationJob) error {
	...
	if err := groupAdapter.DeleteGroup(ctx, group.UpstreamResourceID); err != nil {
		if errors.Is(err, assetadapter.ErrGroupDeletionUnsupported) {
			return model.DB.Transaction(func(tx *gorm.DB) error {
				now := common.GetTimestamp()
				if err := tx.Model(&model.AssetGroupBinding{}).
					...
					Updates(map[string]any{"status": model.AssetBindingStatusDeletionFailed, "error_code": "upstream_group_delete_manual_required", "error_message": "upstream group deletion requires manual handling", "updated_at": now}).Error; err != nil {
					return err
				}
				if err := markRealPersonCleanupStatusTx(tx, group.AuthorizationID, model.RealPersonCleanupManualRequired, "upstream_group_delete_manual_required"); err != nil {
					return err
				}
		...
		if statusErr := model.DB.Transaction(func(tx *gorm.DB) error {
			return markRealPersonCleanupStatusTx(tx, group.AuthorizationID, model.RealPersonCleanupPartial, "upstream_group_delete_failed")
		}); statusErr != nil {
			common.SysError(fmt.Sprintf("failed to persist partial authorization cleanup: job_id=%d group_id=%d err=%v", job.ID, group.ID, statusErr))
		}
		return err
	}
	return completeAssetGroupDeletion(job, &group)
